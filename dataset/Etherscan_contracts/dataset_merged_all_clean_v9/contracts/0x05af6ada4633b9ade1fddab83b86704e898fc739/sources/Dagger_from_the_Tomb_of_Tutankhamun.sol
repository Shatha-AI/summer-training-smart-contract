...
contract Dagger_from_the_Tomb_of_Tutankhamun is
    ERC721AC,
    BasicRoyalties,
    Ownable,
    OwnableBasic
{
    ...
    uint256 public MAX_SUPPLY = 396;
    uint256 public mintStartTime;
    uint256 public mintDuration;
    uint256 public airdropSupply = 330;
...
function airdrop(
    ...
    require(length == quantities.length, "Length mismatch");
    require(length <= 100, "Max 100 addresses");

    uint256 minted = airdropMinted;
    uint256 supply = airdropSupply;

    for (uint256 i; i < length; ) {
        uint256 quantity = quantities[i];

        require(minted + quantity <= supply, "Airdrop supply exceeded");
    ...
    function setAirdropSupply(uint256 _supply) external onlyOwner {
    require(_supply <= MAX_SUPPLY, "Invalid supply");
    require(_supply >= airdropMinted, "Less than minted");
