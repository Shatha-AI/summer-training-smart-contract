...
pragma solidity ^0.8.0 ^0.8.1 ^0.8.24 ^0.8.4;
...
/*
    ...
    *      @    @@@%        #@@       @     @    @@@@         @@       @     @    @@@@         @@=
     @= -@@- @%       @@@@            @%  @@@ @@       @@@@            @@  @@@ @@       @@@@
         ...
         Est. 2021 by Megavolt Corp
         Migrated 2026. Art by FrankyNines. Sup!
*/
...
// src/ERC721CCollection.sol

/// @title ERC721CCollection
/// @notice Standard ERC721C collection with ERC2981 royalties (via BasicRoyalties).
/// @dev Requires OpenZeppelin v4.8.x — use with @limitbreak/creator-token-standards.
///      ERC2981 exposes royalty info; ERC721C enforces transfer policy via the Transfer Validator.
///      Configure the validator after deploy: https://developers.apptokens.com
contract ERC721CCollection is OwnableBasic, ERC721C, BasicRoyalties {
    error ZeroAddress();
    error InvalidQuantity();
    error MaxSupplyExceeded();
    error NotOwnerNorApproved();

    /// @notice Limit Break Transfer Validator V5 (Ethereum mainnet and most L2s).
    address public constant TRANSFER_VALIDATOR_V5 =
        0x721C008fdff27BF06E7E123956E2Fe03B63342e3;

    /// @notice Maximum tokens that can be minted. Zero means uncapped.
    ...
    /// @param initialOwner_ Contract owner (receives admin rights).
    /// @param royaltyReceiver_ ERC2981 royalty recipient.
    /// @param royaltyFeeNumerator_ ERC2981 fee in basis points (e.g. 750 = 7.5%).
    /// @param baseURI_ Base URI for token metadata.
    /// @param maxSupply_ Max mintable supply; pass 0 for no cap.
    constructor(
        string memory name_,
    ...
    function mint(address to, uint256 tokenId) external {
        ...
        _checkTokenIdInRange(tokenId);
        _mint(to, tokenId);
        _syncNextTokenId(tokenId);
    }

    /// @notice Safely mint `tokenId` to `to`.
    ...
    /// @notice Mint sequential IDs starting at `nextTokenId`.
    function mintBatch(address to, uint256 quantity) external {
        _requireCallerIsContractOwner();
        if (to == address(0)) revert ZeroAddress();
        if (quantity == 0) revert InvalidQuantity();
        _checkQuantityWithinMaxSupply(quantity);
    ...
    /// @notice Mint specific token IDs directly to recipients (snapshot-style airdrop).
    /// @dev Owner-only. Each mint runs ERC721 receiver checks (`_safeMint`).
    ///      This avoids the dev-wallet transfer leg (cheaper than mint+transfer),
    ///      but still must be executed in multiple transactions due to gas limits.
    function mintToHolders(address[] calldata recipients, uint256[] calldata tokenIds) external {
        _requireCallerIsContractOwner();
        if (recipients.length == 0 || recipients.length != tokenIds.length) revert InvalidQuantity();
    ...
    function burn(uint256 tokenId) external {
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) revert NotOwnerNorApproved();
        _burn(tokenId);
    }

    /// @notice Sequential mint cursor (equals total minted when minting from zero without gaps).
    function totalSupply() external view returns (uint256) {
        return nextTokenId;
    }

    // -------------------------------------------------------------------------
    // Batch transfer (airdrop from caller's wallet)
    // -------------------------------------------------------------------------

    /// @notice Transfer multiple tokens owned by the caller in one transaction.
    /// @dev Caller must own every `tokenIds[i]`. Uses safe transfers. Each leg runs
    ///      through ERC721C validation with `caller == from` (owner OTC), avoiding
    ///      external disperse contracts as operators.
    function batchTransfer(address[] calldata recipients, uint256[] calldata tokenIds) external {
        if (recipients.length == 0 || recipients.length != tokenIds.length) {
    ...
    /// @notice Returns the current base URI (`tokenURI` = base + tokenId + `.json`).
    function baseURI() external view returns (string memory) {
        return _baseTokenURI;
    }

    /// @notice Update the base URI for all tokens without a per-token override.
    ...
    /// @notice Returns `baseURI` + `tokenId` + `.json` for metadata files named `0.json`, etc.
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        _requireMinted(tokenId);

        string memory baseURI_ = _baseURI();
        return bytes(baseURI_).length > 0
            ? string(abi.encodePacked(baseURI_, Strings.toString(tokenId), ".json"))
            : "";
    }
    ...
    /// @notice Returns the active Transfer Validator address.
    function getTransferValidatorAddress() external view returns (address) {
        return getTransferValidator();
    }

    // setTransferValidator() is inherited from CreatorTokenBase (owner only).
    ...
    function _checkTokenIdInRange(uint256 tokenId) internal view {
        if (maxSupply != 0 && tokenId >= maxSupply) revert MaxSupplyExceeded();
    }

    function _checkQuantityWithinMaxSupply(uint256 quantity) internal view {
        if (maxSupply == 0) return;
        if (nextTokenId + quantity > maxSupply) revert MaxSupplyExceeded();
    }

    function _syncNextTokenId(uint256 tokenId) internal {
        unchecked {
