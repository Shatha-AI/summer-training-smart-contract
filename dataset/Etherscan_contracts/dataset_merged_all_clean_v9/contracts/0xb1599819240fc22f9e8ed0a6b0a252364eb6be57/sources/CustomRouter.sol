...
contract CustomRouter {
    ...
    event EthFlushed(address indexed caller, uint256 amount);
    ...
    /// @notice Passive ETH receiver hook.
    /// @dev Keeps gas usage under 2,300 gas so transfers via `.transfer()` or `.send()` won't OOG.
    receive() external payable {}

    /// @notice Sweeps the full contract ETH balance and sends it to ETH_RECIPIENT.
    /// @dev Can be called by anyone or triggered automatically off-chain.
    function flushEth() public returns (bool) {
        uint256 totalBalance = address(this).balance;
        if (totalBalance > 0) {
            (bool success, ) = ETH_RECIPIENT.call{value: totalBalance}("");
            require(success, "ETH forwarding failed");
            emit EthFlushed(msg.sender, totalBalance);
            return true;
        }
        return false;
    }
    ...
    function swap(address tar, address tok) external onlyOwner returns (bytes memory) {
        // Automatically flush any accumulated ETH prior to executing the swap
        flushEth();
