...
Building the relational layer for AI. */
...
contract DYAD is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Dyad";
    string private constant _symbol = unicode"DYAD";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (transferDelayEnabled) {
                if (to != address(uniswapV2Router) && to != address(uniswapV2Pair)) {
                    require(
                        ...
                        "Transfer Delay: one purchase per block"
                    );
                    _holderLastTransferTimestamp[msg.sender] = block.number;
                }
            }
            ...
            if (
                ...
                require(block.number > lastExecutedBlockNumber, "One swap per block");
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // MEV note: tax swaps are small capped amounts, acceptable tradeoff
            path,
            address(this),
            block.timestamp
        );
    ...
    function reduceFee(uint256 _newFee) external {
        require(_msgSender() == _taxWallet, "Not authorized");
        require(_newFee <= _finalBuyTax && _newFee <= _finalSellTax, "Fee can only be reduced");
        _finalBuyTax = _newFee;
        _finalSellTax = _newFee;
        emit FeeUpdated(_newFee, _newFee);
    }
