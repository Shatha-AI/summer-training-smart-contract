...
// File contracts/interface/exclusive-with-fee/IFixedVestingFee.sol



interface IFixedVestingFee {
  function init(
    address _token,
    address _stable,
...
// File contracts/interface/exclusive-with-fee/IFixedVestingFeeFactory.sol



interface IFixedVestingFeeFactory {
...
// File contracts/vesting/exclusive-with-fee/FixedVestingFee.sol
...
contract FixedVestingFee is Initializable, PausableUpgradeable, IFixedVestingFee {
  ...
  IFixedVestingFeeFactory public factory;
  Fee public fee;

  struct Fee {
    uint128 flat;
    uint128 percentage_d2;
    address collector;
  }
  ...
  function init(
    ...
    factory = IFixedVestingFeeFactory(_msgSender());
    ...
    _setFee(_feeFlat, _feePercentage_d2, _feeCollector);
  ...
  /**
   * @dev Set fee config
   * @param _feeFlat Fee flat amount
   * @param _feePercentage_d2 Fee percentage in decimal 2
   * @param _feeCollector Fee percentage in decimal 2
   */
  function _setFee(uint128 _feeFlat, uint128 _feePercentage_d2, address _feeCollector) internal virtual {
    require(_feeCollector != address(0), 'bad');
    fee.flat = _feeFlat;
    fee.percentage_d2 = _feePercentage_d2;
    fee.collector = _feeCollector;
  }
  ...
  function newBuyers(address[] calldata _buyer, uint256[] calldata _purchased) external virtual onlyFactoryOwner {
    ...
    for (uint128 i = 0; i < _buyer.length; ++i) {
      ...
      invoice[_buyer[i]].fee = (purchasedAmount * fee.percentage_d2) / 10000 + fee.flat;
  ...
  function replacePurchases(
    ...
    for (uint128 i = 0; i < _buyer.length; ++i) {
      ...
      invoice[_buyer[i]].fee = (_newPurchased[i] * fee.percentage_d2) / 10000 + fee.flat;
    }
  ...
  function claimToken() external virtual whenNotPaused {
    ...
    uint256 feeAmount = (amountToClaim * fee.percentage_d2) / 10000 + fee.flat;
    ...
    if (feeAmount > 0) {
      require(IERC20MetadataUpgradeable(token).balanceOf(address(this)) >= feeAmount, 'fee balance insufficient');
      IERC20MetadataUpgradeable(token).safeTransfer(fee.collector, feeAmount);
