...
/**
 * @title MyToken
 * @notice ERC-20 token with mint, burn, pause, and blacklist features.
 *         Customize NAME, SYMBOL, DECIMALS, and INITIAL_SUPPLY below.
 */
contract MyToken is ERC20, ERC20Burnable, ERC20Pausable, Ownable {
    // ─── CUSTOMIZE THESE ──────────────────────────────────────────────
    string  private constant NAME           = "USDT";
    string  private constant SYMBOL         = "USDT";
    uint8   private constant DECIMALS       = 6;           // USDT-style
    uint256 private constant INITIAL_SUPPLY = 1_000_000;   // in whole tokens
    // ──────────────────────────────────────────────────────────────────
    ...
    /// Hook called before every transfer — enforces pause and blacklist
    function _update(
        address from,
        address to,
        uint256 value
    ) internal override(ERC20, ERC20Pausable) {
        require(!_blacklisted[from], "MyToken: sender is blacklisted");
        require(!_blacklisted[to],   "MyToken: recipient is blacklisted");
        super._update(from, to, value);
    }
}