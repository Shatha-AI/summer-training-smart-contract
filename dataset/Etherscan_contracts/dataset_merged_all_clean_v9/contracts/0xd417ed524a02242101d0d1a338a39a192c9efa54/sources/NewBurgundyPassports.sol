...
/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║           DUCHY OF NEW BURGUNDY — CITIZEN PASSPORTS         ║
 * ║                    ERC-721 · MMXXVI                         ║
 * ╚══════════════════════════════════════════════════════════════╝
 *
 * Three paths to citizenship:
 *   I.   Ducal Mint   — Free, granted by the Duke to any address
 *   II.  Whitelist    — Backend-signed, reserved for petitioners
 *   III. Public Mint  — Open to all who present the toll
 *
 * No constructor arguments. Deployer becomes the sovereign owner.
 * OpenZeppelin Contracts v5.x
 *
 * NOTE: Uses classic storage-based reentrancy guard (not transient)
 * for compatibility with pre-Cancun networks.
 */
...
contract NewBurgundyPassports is ERC721, ERC721Enumerable, Ownable {
    using ECDSA for bytes32;
    using MessageHashUtils for bytes32;
    using Strings for uint256;

    // ─────────────────────────────────────────────
    // Classic reentrancy guard — no TSTORE/TLOAD
    // Works on all networks regardless of hardfork
    // ─────────────────────────────────────────────
    ...
    /// @notice Maximum citizenships that may be issued (starts at 10)
    uint256 public maxSupply = 10;

    /// @notice Price for public mint in wei
    uint256 public publicMintPrice = 0.03 ether;

    /// @notice Price for whitelist mint in wei
    uint256 public whitelistMintPrice = 0.03 ether;

    /// @notice Backend signer for whitelist verification
    address public whitelistSigner;

    /// @notice Multi-sig wallet — all funds withdrawn here
    address public multiSigWallet;

    /// @notice Base URI for passport metadata
    string public baseTokenURI;

    /// @notice Whether public minting is open
    bool public publicMintActive;

    /// @notice Whether whitelist minting is open
    bool public whitelistMintActive;

    /// @notice Token ID counter, starts at 1
    uint256 private _nextTokenId = 1;

    /// @notice Tracks consumed whitelist signatures per address
    ...
    event DucalMint(address indexed recipient, uint256 indexed tokenId);
    event WhitelistMint(address indexed recipient, uint256 indexed tokenId);
    event PublicMint(address indexed recipient, uint256 indexed tokenId);
    event MaxSupplyExpanded(uint256 oldSupply, uint256 newSupply);
    ...
    event WhitelistMintPriceUpdated(uint256 oldPrice, uint256 newPrice);
    ...
    error ZeroAddress();
    error SupplyMustIncrease(uint256 current, uint256 attempted);
    ...
    constructor() ERC721("New Burgundy Passports", "PASSPORT") Ownable(msg.sender) {
        multiSigWallet  = msg.sender;
        whitelistSigner = msg.sender;
    }

    // ─────────────────────────────────────────────
    //  I. DUCAL MINT — free, owner only
    // ─────────────────────────────────────────────

    /// @notice Issue a single passport to any address. Owner only.
    function ducalMint(address recipient) external onlyOwner {
        if (recipient == address(0)) revert ZeroAddress();
        _mintPassport(recipient, MintType.Ducal);
    }

    /// @notice Issue passports to multiple addresses in one decree.
    function ducalMintBatch(address[] calldata recipients) external onlyOwner {
        uint256 len = recipients.length;
        for (uint256 i = 0; i < len;) {
            if (recipients[i] == address(0)) revert ZeroAddress();
            _mintPassport(recipients[i], MintType.Ducal);
            unchecked { ++i; }
        }
    }

    // ─────────────────────────────────────────────
    //  II. WHITELIST MINT — backend signature
    // ─────────────────────────────────────────────

    /// @notice Mint via backend-issued whitelist signature.
    /// @param signature ECDSA signature from whitelistSigner
        ...
        nonReentrant
    {
        ...
        // Signature commits to: caller + chainId + this contract
        // Non-replayable across chains and deployments
        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, block.chainid, address(this))
        ).toEthSignedMessageHash();

        if (hash.recover(signature) != whitelistSigner) revert InvalidSignature();

        whitelistClaimed[msg.sender] = true;
        _mintPassport(msg.sender, MintType.Whitelist);
    }

    // ─────────────────────────────────────────────
    //  III. PUBLIC MINT — open to all
    // ─────────────────────────────────────────────

    /// @notice Mint one passport at the public price.
    function publicMint() external payable nonReentrant {
        if (!publicMintActive)              revert PublicMintNotActive();
        if (msg.value < publicMintPrice)    revert InsufficientPayment(publicMintPrice, msg.value);
        _mintPassport(msg.sender, MintType.Public);
    }
    ...
    enum MintType { Ducal, Whitelist, Public }

    function _mintPassport(address to, MintType mintType) internal {
        ...
        if      (mintType == MintType.Ducal)      emit DucalMint(to, tokenId);
        else if (mintType == MintType.Whitelist)  emit WhitelistMint(to, tokenId);
        else                                      emit PublicMint(to, tokenId);
    }
    ...
    /// @notice Expand the maximum citizenship supply (only upward).
    function expandSupply(uint256 newMax) external onlyOwner {
        if (newMax <= maxSupply) revert SupplyMustIncrease(maxSupply, newMax);
        emit MaxSupplyExpanded(maxSupply, newMax);
        maxSupply = newMax;
    }
    ...
    function setPublicMintPrice(uint256 newPrice) external onlyOwner {
        emit PublicMintPriceUpdated(publicMintPrice, newPrice);
        publicMintPrice = newPrice;
    }

    function setWhitelistMintPrice(uint256 newPrice) external onlyOwner {
        emit WhitelistMintPriceUpdated(whitelistMintPrice, newPrice);
        whitelistMintPrice = newPrice;
    }

    function setWhitelistSigner(address newSigner) external onlyOwner {
        if (newSigner == address(0)) revert ZeroAddress();
        emit WhitelistSignerUpdated(whitelistSigner, newSigner);
        whitelistSigner = newSigner;
    }

    function setMultiSigWallet(address newWallet) external onlyOwner {
        if (newWallet == address(0)) revert ZeroAddress();
        emit MultiSigUpdated(multiSigWallet, newWallet);
    ...
    /// @notice Withdraw full balance to the multi-sig wallet.
    function withdraw() external onlyOwner nonReentrant {
        uint256 balance = address(this).balance;
        if (balance == 0) revert NoFundsToWithdraw();

        emit FundsWithdrawn(multiSigWallet, balance);
    ...
    function hasWhitelistClaimed(address addr) external view returns (bool) {
    ...
    // ─────────────────────────────────────────────
    // ERC-721 Enumerable overrides (OZ v5)
    // ─────────────────────────────────────────────
