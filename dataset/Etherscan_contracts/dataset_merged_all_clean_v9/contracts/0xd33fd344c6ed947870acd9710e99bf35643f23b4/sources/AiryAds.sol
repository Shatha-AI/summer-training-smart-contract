 ...
 *
 * @dev      Standard  :  ERC-20 + ERC20Permit
 *           Network   :  Ethereum Mainnet
 *           Supply    :  100,000,000 AIRY (fixed, no mint)
 ...
 *   TOKEN UTILITY
 *
 *   Pay AiryAds subscriptions with $AIRY.
 *   Discount tiers (wallet balance + staked tokens):
 *       Tier 1  >=  1 000 AIRY  —  discount set in AiryAds backend
 *       Tier 2  >=  5 000 AIRY  —  discount set in AiryAds backend
 *       Tier 3  >= 20 000 AIRY  —  discount set in AiryAds backend
 *   7-day staking to boost discount tier.
 *   Referral rewards tracked on-chain.
 *
 * ───────────────────────────────────────────────────────────────────────────────
 *
 *   TOKEN ALLOCATION   Total Supply: 100,000,000 AIRY
 *
 *   Ecosystem         30,000,000   30%   Platform growth and integrations
 *   Public IDO        20,000,000   20%   Public token sale
 *   Liquidity         15,000,000   15%   DEX liquidity
 *   Team              15,000,000   15%   12-month cliff, 36-month linear vest
 *   Investors         10,000,000   10%    6-month cliff, 24-month linear vest
 *   Marketing          7,000,000    7%   Campaigns and community
 *   Reserve            3,000,000    3%   Emergency buffer
 *
 *   Note: Team tokens are held by this contract and released via on-chain
 *   vesting schedule. Investor tokens are sent to the owner wallet at deploy
 *   and assigned to investors via createVesting() during private sale rounds.
 *
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
// ─── CONTRACT ─────────────────────────────────────────────────────────────────
contract AiryAds is ERC20, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    uint256 public constant RENOUNCE_DELAY   = 7 days;
    ...
    bool    public referralLive = false;
    ...
    // ─── CONSTRUCTOR ──────────────────────────────────────────────────────────
    /**
     * @param ecosystem  30% — platform growth
     * @param ido        20% — public sale
     * @param liq        15% — DEX liquidity
     * @param team       15% — 12mo cliff, 36mo linear vest (held by contract)
     * @param investors  10% — sent to owner for private sale, vest via createVesting()
     * @param marketing   7% — campaigns and community
     * @param reserve     3% — emergency buffer
     */
    constructor(
        address ecosystem,
        address ido,
        ...
        Ownable(msg.sender)
    {
        ...
        if (marketing == address(0)) revert ZeroAddress();
        ...
        _mint(marketing,   7_000_000 * 10 ** 18);
        _mint(reserve,     3_000_000 * 10 ** 18);

        _mint(address(this), 15_000_000 * 10 ** 18);
     ...
     *         0 = no tier, 1 = Tier1, 2 = Tier2, 3 = Tier3.
     *         Actual discount percentage is configured in the AiryAds backend.
     * @param  user  Address to evaluate
     * @return tier  0 | 1 | 2 | 3
     */
    function getDiscountTier(address user) external view returns (uint8 tier) {
        uint256 effective = balanceOf(user) + stakedAmt[user];
    ...
    /**
     * @notice Returns circulating supply excluding locked tokens.
     *         Used by CoinGecko, Dexscreener and other trackers.
     *         Circulating = totalSupply - vesting locked - staked
     */
    function circulatingSupply() external view returns (uint256) {
        return totalSupply() - totalVesting - totalStaked;
    }

    /**
     * @notice Returns the timestamp when a user's staked tokens unlock.
     *         Returns 0 if the user has no staked tokens.
     * @param  user  Address to query
     */
    function stakeUnlockTime(address user) external view returns (uint256) {
        if (stakedAmt[user] < 1) return 0;
        return stakeTime[user] + MIN_STAKE_LOCK;
    }
    ...
    /**
     * @notice Lock AIRY for 7 days to boost discount tier.
    ...
    /**
     * @notice Withdraw staked tokens after the 7-day lock.
    ...
    /**
     * @notice Register a referrer on-chain for transparency.
     * @param  ref  Referrer address
     */
    function setReferrer(address ref) external {
        if (!referralLive)                        revert ReferralNotActive();
    ...
    /**
     * @notice Pay a referral reward directly from the owner wallet.
     *         Called by AiryAds backend after a successful subscription payment.
     * @param  ref     Referrer wallet address
     * @param  amount  AIRY amount to pay
     */
    function payReferral(address ref, uint256 amount)
        external nonReentrant onlyOwner
    {
        if (ref == address(0))                     revert ZeroAddress();
        if (amount < 1)                            revert ZeroAmount();
        if (balanceOf(msg.sender) < amount)        revert InsufficientAirdropBalance();
        _transfer(msg.sender, ref, amount);
    ...
    function _createVesting(
        ...
        if (amount < 1)        revert ZeroAmount();
        if (vestingOf[ben].total > 0 && !vestingOf[ben].revoked)
    ...
    /**
     * @notice Total tokens vested so far for a beneficiary.
    ...
    function vestedAmt(address ben) public view returns (uint256) {
        ...
        return (uint256(s.total) * (block.timestamp - s.cliffEnd))
               / (s.vestEnd - s.cliffEnd);
    }
    ...
    /**
     * @notice Seconds until cliff ends (0 if passed).
     * @param  ben  Beneficiary address
     */
     ...
     *         Tokens are held by this contract and released linearly after cliff.
     */
    ...
    function revokeVesting(address ben) external nonReentrant onlyOwner {
        ...
        s.revoked    = true;
        totalVesting -= uint256(s.total) - s.released;
    ...
    /**
     * @notice Create a vesting schedule for a new beneficiary.
     *         Ensure this contract holds sufficient tokens before calling.
     * @param  ben           Beneficiary address
     * @param  amount        Total tokens to vest
     * @param  cliffDuration Cliff period in seconds
     * @param  vestDuration  Vesting duration after cliff in seconds
     * @param  revocable     Whether owner can revoke the schedule
     */
    ...
    /**
     * @notice Enable or disable the referral system.
     ...
     * @param  bps  Rate in basis points (max 1000 = 10%)
     */
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //                      SAFE OWNERSHIP RENOUNCE
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * @notice Queue ownership renouncement. Executes after 7 days.
     */
    function queueRenounce() external onlyOwner {
        renounceReadyAt = block.timestamp + RENOUNCE_DELAY;
        emit RenounceQueued(renounceReadyAt);
    }

    /**
     * @notice Cancel a queued renouncement.
     */
    ...
    /**
     * @notice Execute queued renouncement after the 7-day delay.
     */
    function executeRenounce() external onlyOwner {
        if (renounceReadyAt < 1)
            revert RenounceNotQueued();
        if (block.timestamp < renounceReadyAt)
            revert RenounceNotReady(renounceReadyAt, block.timestamp);
        renounceReadyAt = 0;
    ...
    /**
     * @notice Batch transfer tokens from caller to multiple recipients.
    ...
    function airdrop(
        ...
        uint256 total = 0;
        unchecked {
            for (uint256 i = 0; i < len; ++i) total += amounts[i];
        }
        if (balanceOf(msg.sender) < total) revert InsufficientAirdropBalance();
        unchecked {
     ...
     */
    function rescueERC20(IERC20 token, uint256 amount)
        external nonReentrant onlyOwner
    {
