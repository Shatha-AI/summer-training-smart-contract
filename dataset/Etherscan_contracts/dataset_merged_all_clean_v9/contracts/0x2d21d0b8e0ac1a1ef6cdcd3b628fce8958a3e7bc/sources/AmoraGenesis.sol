...
// File @openzeppelin/contracts-v4/utils/introspection/IERC165.sol@v4.9.6
...
// File @openzeppelin/contracts-v4/interfaces/IERC2981.sol@v4.9.6
...
// File contracts/vendor/seadrop/interfaces/ICreatorToken.sol
...
// File contracts/vendor/seadrop/interfaces/ISeaDropTokenContractMetadata.sol
...
// File contracts/vendor/seadrop/interfaces/ITransferValidator.sol
...
// File contracts/vendor/seadrop/lib/ERC721AConduitPreapproved.sol
...
// File contracts/vendor/seadrop/lib/ERC721TransferValidator.sol
...
// File contracts/vendor/seadrop/lib/utility/ConstructorInitializable.sol
...
// File contracts/vendor/seadrop/lib/utility/TwoStepOwnable.sol
...
// File contracts/vendor/seadrop/ERC721ContractMetadata.sol
...
// File contracts/vendor/seadrop/lib/SeaDropStructs.sol
...
// File contracts/vendor/seadrop/interfaces/INonFungibleSeaDropToken.sol
...
// File contracts/vendor/seadrop/lib/SeaDropErrorsAndEvents.sol
...
// File contracts/vendor/seadrop/interfaces/ISeaDrop.sol
...
// File contracts/vendor/seadrop/lib/ERC721SeaDropStructsErrorsAndEvents.sol
...
// File solmate/src/utils/ReentrancyGuard.sol@v6.8.0
...
// File contracts/vendor/seadrop/ERC721SeaDrop.sol
...
// File contracts/AmoraGenesis.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity 0.8.17;
contract AmoraGenesis is ERC721SeaDrop {
    event TraitUpdated(bytes32 indexed traitKey, uint256 tokenId, bytes32 traitValue);
    event TraitMetadataURIUpdated();

    // ERC-4906: tells marketplaces to re-read this token's metadata. Needed
    // because an awakened Amora's image follows her current mood — without the
    // signal, OpenSea keeps serving a cached frame.
    // BatchMetadataUpdate and the 0x49064906 interface id already come from
    // SeaDrop (ISeaDropTokenContractMetadata / ERC721ContractMetadata); only
    // the single-token event was missing.
    event MetadataUpdate(uint256 _tokenId);
    event RevealCommitted(uint256 indexed tokenId, bytes32 dna);
    event OneOfOneSet(uint256 indexed tokenId);
    event TraitUpdaterSet(address indexed updater);

    error OnlyTraitUpdater();
    error AlreadyRevealed(uint256 tokenId);
    error TeamReserveExceeded();
    error InvalidOneOfOneId(uint256 tokenId);
    error ZeroDna();
    error TraitValueUnchanged();

    uint256 public immutable TEAM_RESERVE;
    uint256 public teamMinted;
    mapping(uint256 => bool) public isOneOfOne;
    mapping(uint256 => bytes32) public revealDna;
    ...
    modifier onlyTraitUpdater() {
        if (msg.sender != traitUpdater) revert OnlyTraitUpdater();
        _;
    }

    function teamMint(address to, uint256 quantity) external onlyOwner {
        if (teamMinted + quantity > TEAM_RESERVE) revert TeamReserveExceeded();
        teamMinted += quantity;
        _safeMint(to, quantity);
    }

    function setOneOfOnes(uint256[] calldata tokenIds) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            if (id == 0 || id > TEAM_RESERVE) revert InvalidOneOfOneId(id);
            if (!isOneOfOne[id]) {
                isOneOfOne[id] = true;
                emit OneOfOneSet(id);
            }
    ...
    function setTraitUpdater(address updater) external onlyOwner {
        traitUpdater = updater;
        emit TraitUpdaterSet(updater);
    }

    function commitReveal(uint256 tokenId, bytes32 dna) external onlyTraitUpdater {
        if (dna == bytes32(0)) revert ZeroDna();
        if (revealDna[tokenId] != bytes32(0)) revert AlreadyRevealed(tokenId);
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        revealDna[tokenId] = dna;
        emit RevealCommitted(tokenId, dna);
        emit MetadataUpdate(tokenId); // unrevealed placeholder -> final artwork
    }

    function isRevealed(uint256 tokenId) external view returns (bool) {
        return revealDna[tokenId] != bytes32(0);
    }

    function setTrait(uint256 tokenId, bytes32 traitKey, bytes32 value)
        external
        onlyTraitUpdater
    {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        if (_traits[tokenId][traitKey] == value) revert TraitValueUnchanged();
        _traits[tokenId][traitKey] = value;
        emit TraitUpdated(traitKey, tokenId, value);
        emit MetadataUpdate(tokenId);
    }

    /// @notice Batch trait write. Moods are synced hourly, and one transaction
    /// per token is wasteful: the 21k base cost is shared across the batch.
    /// Entries whose value already matches are skipped rather than reverting,
    /// so a single unchanged token cannot cancel the whole update.
    function setTraitBatch(
        uint256[] calldata tokenIds,
        bytes32 traitKey,
        bytes32[] calldata values
    ) external onlyTraitUpdater {
        if (tokenIds.length != values.length) revert TraitValueUnchanged();
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if (!_exists(tokenId)) continue;
            if (_traits[tokenId][traitKey] == values[i]) continue;
            _traits[tokenId][traitKey] = values[i];
            emit TraitUpdated(traitKey, tokenId, values[i]);
            emit MetadataUpdate(tokenId);
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        // 0xaf332f3e — ERC-7496 (dynamic traits).
        // ERC-4906 (0x49064906) is already reported by super (ERC721ContractMetadata).
        return interfaceId == bytes4(0xaf332f3e) || super.supportsInterface(interfaceId);
    }
}