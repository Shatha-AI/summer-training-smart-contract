...
/*
    ...
    Twitter/X : https://x.com/elonmusk/status/2082613281328660734
    Telegram  : https://t.me/miniharderc20
...
contract Token is Context, IERC20, Ownable {
    ...
    // Hard ceiling for updateBuyTax/updateSellTax. Constant, so it can never be
    // raised after deploy — the final tax can move, but never above this.
    uint256 private constant _maxFee = 10;

    uint8 private constant _decimals = 9;
    uint256 private constant _tTotal = 1000000000 * 10**_decimals;
    string private constant _name = unicode"Minihard";
    string private constant _symbol = unicode"MINIHARD";
    ...
    constructor (address payable taxWallet_) {
    ...
    // Set the final buy/sell tax after launch (utility tokens run 5/5, memes 0/0).
    // Capped at _maxFee (10%) and gated on the tax wallet rather than the owner,
    // so it keeps working after renouncing ownership.
    function updateBuyTax(uint256 newBuyTax) external {
        require(_msgSender()==_taxWallet);
        require(newBuyTax<=_maxFee, "Buy tax too high");
        _finalBuyTax=newBuyTax;
    }

    function updateSellTax(uint256 newSellTax) external {
        require(_msgSender()==_taxWallet);
        require(newSellTax<=_maxFee, "Sell tax too high");
