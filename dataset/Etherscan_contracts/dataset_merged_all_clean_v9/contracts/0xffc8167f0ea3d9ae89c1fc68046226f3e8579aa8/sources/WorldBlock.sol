...
contract WorldBlock is ERC721, Ownable {
    // نسبة عمولة المنصة (10%)
    uint256 public constant ROYALTY_FEE_PERCENT = 10;
    
    struct Country {
        string currentName;
        uint256 price; // السعر بالـ (Wei) الذي يحدده المستخدم للبيع
        bool isMinted; // هل تم شراء الدولة مسبقاً؟
    }

    // ربط رقم الدولة (معرف الـ NFT) ببيانات الدولة
    mapping(uint256 => Country) public countries;

    // أحداث لتسجيل العمليات على محرك البلوكشين مباشرة
    event CountryMinted(uint256 indexed countryId, address owner, uint256 price);
    event CountryRenamed(uint256 indexed countryId, string newName);
    event CountryPriceChanged(uint256 indexed countryId, uint256 newPrice);
    event CountrySold(uint256 indexed countryId, address from, address to, uint256 price);

    // دالة البناء: تعينك أنت مباشرة كمالك العقد (صاحب المنصة) لأنك من سيرفعه
    constructor() ERC721("WorldBlock Territory", "WBT") Ownable(msg.sender) {}

    // 1. الدالة الأولى: الشراء لأول مرة (أموالها بالكامل تذهب لك)
    function mintCountry(uint256 _countryId, string memory _name) public payable {
        require(!countries[_countryId].isMinted, "This country is already owned");
        require(msg.value > 0, "Must pay the initial price to mint");

        // تسجيل بيانات الدولة الجديدة في العقد
        countries[_countryId] = Country({
            currentName: _name,
            price: 0, // بعد الشراء الأول، الدولة لا تُعرض للبيع إلا إذا طلب المالك ذلك
            isMinted: true
        });

        // إصدار الـ NFT لصالح المشتري
        _safeMint(msg.sender, _countryId);

        // **تحويل الأموال بالكامل إلى محفظتك أنت (مالك العقد)**
        (bool success, ) = owner().call{value: msg.value}("");
        require(success, "Transfer to owner failed");

        emit CountryMinted(_countryId, msg.sender, msg.value);
    }

    // 2. تغيير اسم الدولة (فقط المالك الحالي يحق له ذلك)
    function setCountryName(uint256 _countryId, string memory _newName) public {
        require(ownerOf(_countryId) == msg.sender, "You do not own this country");
        countries[_countryId].currentName = _newName;
        emit CountryRenamed(_countryId, _newName);
    }

    // 3. عرض الدولة للبيع مرة أخرى (يحدد المالك السعر)
    function setCountryPrice(uint256 _countryId, uint256 _newPrice) public {
        require(ownerOf(_countryId) == msg.sender, "You do not own this country");
        countries[_countryId].price = _newPrice;
        emit CountryPriceChanged(_countryId, _newPrice);
    }

    // 4. الدالة الأهم: البيع في السوق الثانوي (بين المستخدمين) بنسبة مقتطعة
    function buyCountry(uint256 _countryId) public payable {
        require(countries[_countryId].isMinted, "Country not minted yet");
        uint256 price = countries[_countryId].price;
        require(price > 0, "Country is not for sale by owner");
        require(msg.value >= price, "Insufficient funds sent");

        address currentOwner = ownerOf(_countryId);
        require(currentOwner != msg.sender, "You already own this country");

        // حساب 10% لك كصاحب منصة، و90% للبائع
        uint256 royaltyFee = (msg.value * ROYALTY_FEE_PERCENT) / 100;
        uint256 sellerAmount = msg.value - royaltyFee;

        // إزالة الدولة من العرض بعد البيع (تصفير السعر)
        countries[_countryId].price = 0;

        // نقل ملكية الدولة من المالك القديم إلى الجديد
        _transfer(currentOwner, msg.sender, _countryId);

        // تحويل 10% لك (إلى محفظة صاحب العقد)
        (bool successRoyalty, ) = owner().call{value: royaltyFee}("");
        require(successRoyalty, "Royalty transfer failed");

        // تحويل 90% للبائع
        (bool successSeller, ) = payable(currentOwner).call{value: sellerAmount}("");
        require(successSeller, "Seller transfer failed");

        emit CountrySold(_countryId, currentOwner, msg.sender, msg.value);
    }
}