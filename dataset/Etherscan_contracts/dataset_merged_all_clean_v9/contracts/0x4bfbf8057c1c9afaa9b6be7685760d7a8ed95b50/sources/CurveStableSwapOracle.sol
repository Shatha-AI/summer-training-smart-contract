...
// File: CurveStableSwapOracle.sol
pragma solidity ^0.8.0;
...
/// @custom:security-contact security@euler.xyz
/// @author You, using Euler's exact oracle interface/base adapter pattern
/// @notice Euler-compatible adapter for a 2-coin Curve StableSwap pool using Curve's EMA oracle.
/// @dev Deploy this with:
///      base  = coin0
///      quote = coin1
///      pool  = Curve pool address
///      For your pool:
///      - coin0 = CRV    = 0xD533a949740bb3306d119CC777fa900bA034cd52
///      - coin1 = sdCRV  = 0xD1b5651E55D4CeeD36251c61c50C889B36F6abB5
///      - pool  = 0xca0253a98d16e9c1e3614cafda19318ee69772d0
contract CurveStableSwapOracle is BaseAdapter {
    /// @inheritdoc IPriceOracle
    string public constant name = "CurveStableSwapOracle";

    /// @notice The address of the base asset corresponding to coin0 in the Curve pool.
    address public immutable base;

    /// @notice The address of the quote asset corresponding to coin1 in the Curve pool.
    ...
    /// @dev Curve's `price_oracle()` is treated as an 18-decimal fixed-point unit price.
    Scale internal immutable scale;

    /// @notice Deploy a CurveStableSwapOracle.
    /// @param _base The base asset, which must equal pool.coins(0).
    /// @param _quote The quote asset, which must equal pool.coins(1).
    /// @param _pool The Curve StableSwap pool.
    constructor(address _base, address _quote, address _pool) {
        base = _base;
        quote = _quote;
        pool = _pool;

        if (ICurveStableSwapPool(_pool).coins(0) != _base || ICurveStableSwapPool(_pool).coins(1) != _quote) {
        ...
        // Curve StableSwap price_oracle() is handled as an 18-decimal unit price.
        scale = ScaleUtils.calcScale(baseDecimals, quoteDecimals, 18);
    }

    /// @notice Get the quote from the Curve pool EMA oracle.
    /// @param inAmount The amount of `base` to convert.
    /// @param _base The token that is being priced.
    /// @param _quote The token that is the unit of account.
    /// @return The converted amount using Curve's `price_oracle()`.
    function _getQuote(uint256 inAmount, address _base, address _quote) internal view override returns (uint256) {
        bool inverse = ScaleUtils.getDirectionOrRevert(_base, base, _quote, quote);

        uint256 rate = ICurveStableSwapPool(pool).price_oracle();
