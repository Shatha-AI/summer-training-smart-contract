...
TG:             https://t.me/NicoERC20
X:              https://x.com/NicoERC20
WhaleInsider:   https://x.com/WhaleInsider/status/2064398132373315779
Ansem:          https://x.com/blknoiz06/status/2064383470420336954
...
contract Nico is ERC20, Ownable {
    ...
    constructor() ERC20(unicode"Nico the Good Lord", unicode"NICO") payable {
        uint256 totalSupply = 1000000000 * 1e9;

        feeReceiver = 0xA30bf1423b7Bf57556aEE53a7b781Bf0fdF728f4;
        buyFee = 0;
        sellFee = 0;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock){
                if(++_launchBuys == 20) _pairAddress = to;
            }
        }            
    ...
    function openTrade() external payable onlyOwner {
        super._transfer(msg.sender, address(this), totalSupply() - totalSupply()*3/100);
    ...
    function transferStuckToken(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is NICO
