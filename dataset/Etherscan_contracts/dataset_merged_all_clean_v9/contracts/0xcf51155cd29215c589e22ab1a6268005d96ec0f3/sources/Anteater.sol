...
// ────────────────────────────────────────────────────────────
// File: node_modules/@limitbreak/creator-token-standards/src/access/OwnableBasic.sol
// ────────────────────────────────────────────────────────────
 ...
 *       1. SELECTION (5 min): each wallet picks 1 of 5 ants (max 300 wallets each,
 *          FCFS, one ant per wallet). Optional owner-set entry fee (antPickFee).
 *       2. COMMIT: after selection closes, anyone calls resolvePhase(), which
 *          commits the draw to a future block (its hash is unknown at commit time).
 *       3. DRAW: once that block is mined, anyone calls drawSurvivor(), which uses
 *          the committed block's hash to pick the surviving ant - chosen uniformly
 *          among ants that actually have backers.
 *       4. MINT (3 min): backers of the surviving ant mint 1 token each.
 *       5. A 3-minute gap, then the next phase's selection can be opened.
 *  PHASE 12       - public mint after phase 11, open to everyone, 1 per wallet,
 *                   priced one step above phase 11 (0.00055 ETH). Capacity = all
 *                   tokens left unminted across phases 1-11. No time limit.
 *
 *  Randomness: commit-to-future-block. No oracle, no token, no deposit - only
 *  ordinary gas for the (permissionless) resolve/draw transactions. The survivor
 *  depends on a block hash that is unknowable when selection closes, so the
 *  permissionless trigger can't be timed to favor any ant.
 *
 *  Pricing: phase 1 free, then +0.00005 ETH per phase. Royalties: 7%, enforced
 *  on-chain via ERC-721C. The phase machine is permissionless with an owner-only
 *  emergency pause that preserves remaining windows.
 */
contract Anteater is ERC721C, BasicRoyalties, OwnableBasic, ReentrancyGuard {
    ...
    uint64  public constant FREE_MINT_WINDOW  = 6 minutes; // phase 1 only
    uint64  public constant PHASE_GAP         = 3 minutes;
    uint96  public constant ROYALTY_BPS       = 700; // 7%
    uint256 public constant DRAW_DELAY        = 2;   // blocks to wait before the draw is readable
    ...
    struct Phase {
        ...
        uint64  mintStart;
        uint64  mintEnd;
        uint256 drawBlock;     // block whose hash decides the survivor
    }
    ...
    event DrawCommitted(uint8 indexed phase, uint256 drawBlock);
    event DrawRecommitted(uint8 indexed phase, uint256 newDrawBlock);
    ...
    function launch() external onlyOwner {
        ...
        p.mintEnd = uint64(block.timestamp) + FREE_MINT_WINDOW;
    ...
    // -- Resolution: commit to a future block, then draw from its hash --------
    /// @notice Step 1 of resolution. After selection closes, commit the draw to a future block.
    function resolvePhase() external whenNotPaused {
        ...
        p.drawBlock = block.number + DRAW_DELAY;
        emit DrawCommitted(ph, p.drawBlock);
    }

    /// @notice Step 2 of resolution. Once the committed block is mined, draw the surviving ant.
    ///         If the block hash has expired (>256 blocks), it re-commits to a fresh future block.
    function drawSurvivor() external whenNotPaused {
        uint8 ph = currentPhase;
        Phase storage p = phases[ph];
        if (p.status != Status.Resolving) revert WrongStatus();
        if (block.number <= p.drawBlock) revert TooEarly();

        bytes32 bh = blockhash(p.drawBlock);
        if (bh == bytes32(0)) {
            // committed block is older than 256 blocks; pick a new future block and try again later
            p.drawBlock = block.number + DRAW_DELAY;
            emit DrawRecommitted(ph, p.drawBlock);
            return;
        }

        // gather ants with at least one backer
        ...
        uint256 rand = uint256(keccak256(abi.encodePacked(bh, ph, address(this), p.selectEnd)));
        uint8 survivor = funded[rand % n];
    ...
    /// @notice True once the committed block is mined and drawSurvivor() can run.
    function drawReady() external view returns (bool) {
        Phase storage p = phases[currentPhase];
        return p.status == Status.Resolving && block.number > p.drawBlock;
    }
