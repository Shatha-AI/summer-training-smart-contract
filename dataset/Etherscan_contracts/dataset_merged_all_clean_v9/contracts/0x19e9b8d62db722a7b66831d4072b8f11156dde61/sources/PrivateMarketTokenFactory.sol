...
pragma solidity =0.8.33 ^0.8.0 ^0.8.1 ^0.8.2 ^0.8.4;
...
// contracts/external/openzeppelin/contracts/proxy/IERC1967.sol
...
// contracts/globalMarkets/gmTokenCompliance/IOndoComplianceGMView.sol
...
// contracts/globalMarkets/tokenFactory/registrars/IRegistrar.sol
...
// contracts/globalMarkets/tokenPauseManager/ITokenPauseManager.sol
...
// contracts/external/openzeppelin/contracts/beacon/UpgradeableBeacon.sol
...
// contracts/external/openzeppelin/contracts/beacon/BeaconProxy.sol
...
// contracts/globalMarkets/PrivateMarketToken.sol
/**
...
/**
 * @title  PrivateMarketToken
 * @author Ondo Finance
 * @notice Private Market token implementation with compliance and pause functionality. These
 *         tokens will follow the BeaconProxy pattern for ease of global upgrades.
 */
contract PrivateMarketToken is
  ...
  TokenPauseManagerClientUpgradeable
{
  ...
  /**
   * @notice Initializes the PrivateMarketToken contract
  ...
  function initialize(
    ...
    __privateMarketToken_init(_nameOverride, _symbolOverride, _compliance, _tokenPauseManager);
  }

  /**
   * @notice Internal initialization function for PrivateMarketToken
   * @param  _nameOverride      The initial name of the token
   * @param  _symbolOverride    The initial symbol of the token
   * @param  _compliance        The address of the compliance contract
   * @param  _tokenPauseManager The address of the token pause manager contract
   * @dev    Initializes all parent contracts and sets up the PrivateMarketToken specific state
   */
  function __privateMarketToken_init(
    string memory _nameOverride,
    string memory _symbolOverride,
    address _compliance,
    address _tokenPauseManager
    ...
    __TokenPauseManagerClientInitializable_init_unchained(_tokenPauseManager);
    __privateMarketToken_init_unchained(_nameOverride, _symbolOverride);
  }

  /**
   * @notice Unchained initialization function for PrivateMarketToken-specific state
   * @param  _nameOverride   The initial name of the token
   * @param  _symbolOverride The initial symbol of the token
   * @dev    Sets up PrivateMarketToken-specific state without calling parent initializers
   */
  function __privateMarketToken_init_unchained(
    string memory _nameOverride,
    string memory _symbolOverride
  ) internal onlyInitializing {
...
// contracts/globalMarkets/tokenFactory/PrivateMarketTokenFactory.sol
/**
...
/**
 * @title  PrivateMarketTokenFactory
 * @author Ondo Finance
 * @notice This contract serves as a factory for deploying and configuring Private Market tokens with
 *         built-in compliance and pause management.
 *
 *         This contract allows for:
 *         - Deploying new Private Market tokens with preconfigured compliance and pause management
 ...
 */
contract PrivateMarketTokenFactory is AccessControlEnumerable, ReentrancyGuard, Pausable {
  ...
  /**
   * @notice Emitted when a new token is deployed (regardless of registration)
   ...
   * @dev    Intentionally kept named `NewGMTokenDeployed` (identical name + signature to
   *         `GMTokenFactory`) so its topic hash is unchanged and existing subgraphs index tokens
   *         from this factory without any ABI/handler modification.
   */
  ...
  constructor(
    ...
    address privateMarketTokenImplementation = address(new PrivateMarketToken());
    UpgradeableBeacon _beaconContract = new UpgradeableBeacon(privateMarketTokenImplementation);
  ...
  /**
   * @notice Deploys a new token and registers it with both bridge and token manager
   * @param  name       The name of the token
   * @param  ticker     The token symbol
  ...
  function deployAndRegisterToken(string calldata name, string calldata ticker, address tokenAdmin)
    ...
    returns (address)
  {
    PrivateMarketToken privateMarketToken =
      PrivateMarketToken(_deployPrivateMarketToken(name, ticker));

    privateMarketToken.grantRole(DEFAULT_ADMIN_ROLE, address(bridgeRegistrar));
    bridgeRegistrar.register(address(privateMarketToken));
    privateMarketToken.revokeRole(DEFAULT_ADMIN_ROLE, address(bridgeRegistrar));

    privateMarketToken.grantRole(DEFAULT_ADMIN_ROLE, address(tokenManagerRegistrar));
    tokenManagerRegistrar.register(address(privateMarketToken));
    privateMarketToken.revokeRole(DEFAULT_ADMIN_ROLE, address(tokenManagerRegistrar));

    privateMarketToken.grantRole(DEFAULT_ADMIN_ROLE, tokenAdmin);
    privateMarketToken.renounceRole(DEFAULT_ADMIN_ROLE, address(this));

    return address(privateMarketToken);
  }

  /**
   * @notice Deploys a new token without registering it anywhere
   * @param  name       The name of the token
   * @param  ticker     The token symbol
   * @param  tokenAdmin The address that will receive admin rights on the token
   * @return            The address of the deployed token proxy
   ...
   * @return        The address of the deployed token proxy
   */
  function _deployPrivateMarketToken(string calldata name, string calldata ticker)
    internal
    returns (address)
  {
    if (tickerExists[ticker]) revert TickerAlreadyExists();
    BeaconProxy privateMarketTokenProxy = new BeaconProxy(beacon, "");
    PrivateMarketToken privateMarketTokenProxied =
      PrivateMarketToken(address(privateMarketTokenProxy));
    privateMarketTokenProxied.initialize(name, ticker, compliance, tokenPauseManager);
    tickerExists[ticker] = true;
    emit TickerSet(ticker, true);

    emit NewGMTokenDeployed(
      address(privateMarketTokenProxied), beacon, name, ticker, compliance, tokenPauseManager
    );

    return address(privateMarketTokenProxied);
  }
