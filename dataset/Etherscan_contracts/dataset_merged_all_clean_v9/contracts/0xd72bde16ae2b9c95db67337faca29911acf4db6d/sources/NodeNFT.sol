 ...
 * @title 节点 NFT
 * @author suni
 * @notice 私募节点凭证,总量上限 50;Enumerable 便于 NFT 池按持有者均分(有界循环)
 */
contract NodeNFT is ERC721, ERC721Enumerable, Ownable, INodeNFT {
    /// @notice 节点总量上限(20~30,可配置,默认 30)
    uint256 public maxNodes = 30;

    /// @notice 允许铸造的地址(如 IDOSale)
    mapping(address => bool) public minters;

    uint256 private _nextId;

    constructor() ERC721("suni Node", "sNODE") Ownable(msg.sender) {}
    ...
    function totalSupply() public view override(ERC721Enumerable, INodeNFT) returns (uint256) {
        return ERC721Enumerable.totalSupply();
    }

    function tokenByIndex(uint256 index) public view override(ERC721Enumerable, INodeNFT) returns (uint256) {
        return ERC721Enumerable.tokenByIndex(index);
    }

    function ownerOf(uint256 tokenId) public view override(ERC721, IERC721, INodeNFT) returns (address) {
