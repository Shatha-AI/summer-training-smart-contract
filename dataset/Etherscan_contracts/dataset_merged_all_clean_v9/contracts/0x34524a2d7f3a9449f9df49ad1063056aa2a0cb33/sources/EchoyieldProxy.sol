...
// File: contracts/EchoyieldProxy.sol


pragma solidity ^0.8.20;


/// @title EchoyieldProxy
/// @notice Deployable ERC-1967 proxy that sits in front of the `EchoyieldDeposit09`
///         logic (implementation) contract and delegatecalls into it, while holding
///         all state itself. This is the address users interact with.
///
/// @dev Deploy in this order:
///        1. deploy `EchoyieldDeposit09` (the implementation)
///        2. deploy `EchoyieldProxy(implementation, initData)` where
///           `initData = abi.encodeCall(EchoyieldDeposit09.initialize, ())`
///      Passing `initData` runs `initialize()` in the SAME transaction as the
///      proxy deployment, which prevents anyone from front-running initialization
///      and seizing ownership.
///
///      Upgrades are performed against this same proxy address via the
///      implementation's UUPS `upgradeToAndCall`, gated by `onlyOwner`.
contract EchoyieldProxy is ERC1967Proxy {
