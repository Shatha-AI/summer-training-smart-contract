...
contract DistributionVaultV3 {
    ...
    function _distribute(address caller, uint256 balance) internal {
        ...
        //   Swap USDT_GAS_REFUND USDT → WETH → ETH and send to caller.
        //   This reduces the contract's USDT balance by USDT_GAS_REFUND.
        //   Recipient payouts below are computed from the FULL pre-swap
        //   `balance`, so all 11 named recipients receive their documented
        //   exact amounts.  Only the owner remainder absorbs the gas cost.
        if (balance > USDT_GAS_REFUND) {
        ...
        // ── Step 2: Named recipients ─────────────────────────────────────
        //   Payouts use the original `balance` (before gas deduction) so
        //   each recipient receives exactly (balance × share) / DENOM USDT
        //   as documented in the NatSpec and distribution schedule.
        for (uint256 i = 0; i < RECIPIENT_COUNT; i++) {
            uint256 payout = (balance * shares[i]) / DENOM;
            if (payout > 0) {
        ...
        // ── Step 3: Owner remainder ──────────────────────────────────────
        //   Remainder = balanceOf(this) after the gas swap and recipient
        //   payouts = (balance × 45001/100000) − USDT_GAS_REFUND.
        ...
        )
    {
        ...
        // Recipient payouts are computed on the FULL balance (gas refund only
        // reduces the owner portion, not named-recipient amounts).
        uint256 totalRecipientPayout;
        for (uint256 i = 0; i < RECIPIENT_COUNT; i++) {
            amounts[i] = (bal * shares[i]) / DENOM;
            totalRecipientPayout += amounts[i];
        }
        // Owner gets the full remainder minus the gas-refund slice.
        uint256 fullOwnerRemainder = bal - totalRecipientPayout;
        ownerAmount = fullOwnerRemainder > USDT_GAS_REFUND
            ? fullOwnerRemainder - USDT_GAS_REFUND
            : 0;
    }
     ...
     *                       (full owner share minus USDT_GAS_REFUND).
     */
        ...
        )
    {
        ...
        uint256 totalRecipientPayout;
        for (uint256 i = 0; i < RECIPIENT_COUNT; i++) {
            amounts[i] = (hypotheticalBalance * shares[i]) / DENOM;
            totalRecipientPayout += amounts[i];
        }
        uint256 fullOwnerRemainder = hypotheticalBalance - totalRecipientPayout;
        ownerAmount = fullOwnerRemainder > USDT_GAS_REFUND
            ? fullOwnerRemainder - USDT_GAS_REFUND
            : 0;
    }
