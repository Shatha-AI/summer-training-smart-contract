...
// File: contracts/contracts/src/Registry.sol
...
/**
 * @title Registry：邀请关系 + 团队等级（v3 合并 · UUPS 可升级）
 * @author suni
 * @notice 合并 ReferralRegistry + TeamLevel：
 *         - 单指针邀请树 + 激活；用户可自助 bindUpline，运营 bind/adminBind 补录
 *         - 综合业绩沿上级链传播(≤maxUplineDepth)，V1–V6 三项阈值同时满足才达级
 */
contract Registry is IRegistry, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @notice 上行遍历深度上限(gas 兜底)
    uint256 public override maxUplineDepth;

    // ---- 团队等级 ----
    mapping(address => uint256) public personalPerf; // 个人业绩
    mapping(address => uint256) public directPerf; // 直推业绩
    ...
    /// @notice 共振池回调(RewardHub)
    IRewardHub public resonanceHook;
     ...
     * @dev 上行遍历受 maxUplineDepth 限制;祖先跨入 V5 时回调共振池登记
     */
    ...
    function _maybeResonance(address user, uint8 levelBefore) internal {
        if (address(resonanceHook) == address(0) || levelBefore >= 5) return;
        if (levelOf(user) >= 5) {
            (bool ok,) =
                address(resonanceHook).call(abi.encodeWithSignature("onQualified(address)", user));
            ok;
