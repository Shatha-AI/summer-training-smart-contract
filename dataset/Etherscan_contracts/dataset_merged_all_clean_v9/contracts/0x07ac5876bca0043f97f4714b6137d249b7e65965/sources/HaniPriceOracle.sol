...
// OpenZeppelin Contracts v4.9.3 (access/AccessControl.sol)
...
/**
 * @title HaniPriceOracle
 * @dev Oracle contract that provides EUR/HANI exchange rate
 */
contract HaniPriceOracle is AccessControl {
    ...
    uint256 public maxHistoryPoints = 1000;
    ...
    function setPriceUpdateInterval(uint256 _interval) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit PriceUpdateIntervalChanged(oldInterval, _interval);
    }
    ...
    function eurToTokens(uint256 amountEur) external view returns (uint256) {
        return (amountEur * 10**18) / currentPrice;
    }
    
    function tokensToEur(uint256 tokenAmount) external view returns (uint256) {
        return (tokenAmount * currentPrice) / 10**18;
    }
    
    function getPriceHistory(uint256 count) external view returns (uint256[] memory prices, uint256[] memory timestamps) {
        uint256 historyCount = priceHistory.length;
        uint256 resultCount = count > historyCount ? historyCount : count;
        
        prices = new uint256[](resultCount);
        timestamps = new uint256[](resultCount);
        
        for (uint256 i = 0; i < resultCount; i++) {
            uint256 idx = historyCount - resultCount + i;
