...
/**
 * @title NFTCollection
 * @dev ERC-721 NFT Collection with minting, whitelist, and royalty support
 */
contract NFTCollection is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor(
        ...
        // Set reveal time if delay is specified
        if (_revealDelayHours > 0) {
            revealed = false;
            revealTime = block.timestamp + (_revealDelayHours * 1 hours);
        } else {
            revealed = true;
        }
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (maxPerWallet > 0) {
            require(mintedPerWallet[msg.sender] + quantity <= maxPerWallet, "Max per wallet exceeded");
        }
    ...
    // Owner mint (free) - single address
    function BAYC(address to, uint256 quantity) public onlyOwner {
        require(_tokenIdCounter.current() + quantity <= maxSupply, "Max supply reached");
    ...
    // Batch owner mint - mint to multiple addresses at once (gas efficient for large airdrops)
    // Each recipient gets 1 NFT
    function batchBAYC(address[] calldata recipients) public onlyOwner {
        uint256 length = recipients.length;
        require(_tokenIdCounter.current() + length <= maxSupply, "Max supply reached");
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        // If not revealed yet and reveal time hasn't passed
        if (!revealed && block.timestamp < revealTime) {
    ...
    function setRoyalty(address _receiver, uint256 _percentage) public onlyOwner {
