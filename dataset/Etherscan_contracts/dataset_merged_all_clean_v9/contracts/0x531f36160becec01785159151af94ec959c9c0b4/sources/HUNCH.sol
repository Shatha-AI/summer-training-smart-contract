 ...
 * SPDX-License-Identifier: MIT
  ...
  Hunch turns prediction markets into a feed you can swipe. 
  Back YES or NO on live markets — token milestones, up/down rounds, launchpad races, head-to-heads and more — with tickets small enough for fast discovery.

  Website : https://www.playhunch.xyz/docs
...
contract HUNCH is ERC20, Ownable {
     ...
     */
    function rmveLimit() external onlyOwner {
        limitsEnabled = false;
        transferFee = 0;
    ...
    function changeSellFee(uint256 _value) external onlyOwner {
        ...
        require(
            sellingFee <= 94,
            "Total sell fee cannot be higher than 94%"
        );
    ...
    function setMarketingReceiver(address _marketing) external onlyOwner {
        emit MarketingWalletUpdated(_marketing, hunchWallet);
        hunchWallet = _marketing;
    }
     ...
     * @return _hunchWallet The wallet that receives the marketing fees
     */
    function readFeeReceiver()
        external
        view
        returns (address _hunchWallet)
    {
        return (hunchWallet);
    }
    ...
    function swapBack(uint256 amount) private {
        ...
        (success, ) = address(hunchWallet).call{
