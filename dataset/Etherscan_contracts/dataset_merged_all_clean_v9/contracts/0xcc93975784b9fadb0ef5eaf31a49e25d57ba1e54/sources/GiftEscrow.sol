 ...
 *         Uses a commit-reveal scheme for MEV protection.
 */
contract GiftEscrow is IERC721Receiver, ReentrancyGuard {
    ...
    /// @notice Per-address commitments: codeHash => claimer => Commitment
    /// @dev Each wallet has its own independent commitment slot.
    ///      Multiple wallets can commit simultaneously without blocking each other.
    ///      Only the wallet that knows the real secret code can reveal and claim.
    mapping(bytes32 => mapping(address => Commitment)) public claimCommitments;

    /// @notice approve(address,uint256) selector
    ...
    /// @notice Maximum number of blocks before a commitment expires (~50 min on mainnet)
    ...
    /// @notice Rejects plain ETH transfers to prevent accidental loss.
    ///         ETH gifts must be sent through createAndLockGiftBatch.
    ...
    function commitClaim(bytes32 codeHash, bytes32 commitment) external {
        ...
        claimCommitments[codeHash][msg.sender] = Commitment({
            value: commitment,
    ...
    function revealAndClaim(bytes32 codeHash, string calldata code, bytes32 nonce) external nonReentrant {
        ...
        Commitment storage commit = claimCommitments[codeHash][msg.sender];
        ...
        pack.claimed = true;
        delete claimCommitments[codeHash][msg.sender];
    ...
    function createAndLockGiftBatch(
        ...
        for (uint256 i = 0; i < approvals.length; i++) {
            require(approvals[i].data.length >= 4, "Invalid approval data");
            bytes4 selector = bytes4(approvals[i].data[:4]);
            require(selector == APPROVE_SELECTOR, "Only approve() calls allowed");

            require(approvals[i].data.length >= 36, "Invalid approval data length");
            bytes memory approvalData = approvals[i].data;
            address spender;
            assembly {
                spender := mload(add(approvalData, 0x24))
            }
            require(spender == address(this), "Spender must be this contract");

            bool tokenFound = false;
            for (uint256 j = 0; j < assets.length; j++) {
                if (assets[j].tokenAddress == approvals[i].tokenAddress) {
                    tokenFound = true;
                    break;
                }
            }
            require(tokenFound, "Approval token not in asset list");
        ...
        emit GiftPackLocked(codeHash, msg.sender, assets.length);
    }
