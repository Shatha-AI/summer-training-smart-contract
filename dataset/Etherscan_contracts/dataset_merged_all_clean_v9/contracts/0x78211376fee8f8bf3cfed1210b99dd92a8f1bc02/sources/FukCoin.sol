 ...
 *   |                                          |
 *   |   FFFFF  U   U  K   K      EST 2026      |
 *   |   F      U   U  K  K    --------------   |
 *   |   FFF    U   U  KKK       C  O  I  N     |
 *   |   F      U   U  K  K    --------------   |
 *   |   F       UUU   K   K                    |
 *   |                                          |
 *   |        MAX SUPPLY : 77,777,777           |
 *   |    "The only coin that gives a FUK"      |
 *   |           www.cryptofuks.com             |
 *   |__________________________________________|
 *
 */
...
contract FukCoin is ERC20, ERC20Burnable, ERC20Permit, Ownable {
    using SafeERC20 for IERC20;

    // Fixed Supply of 77,777,777
    uint256 public constant MAX_SUPPLY = 77777777 * 10 ** 18;
    ...
    constructor()
        ...
        Ownable(msg.sender)
    {
        // All 77,777,777 FUK coins go to deployer wallet instantly at deployment only
        _mint(msg.sender, MAX_SUPPLY);
    }

    /**
     * @dev Rescue tokens sent here by mistake.
     */
    function sweep(address token, uint256 amount) external onlyOwner {
        require(token != address(0), "Invalid address");
        require(token != address(this), "Cannot sweep FUK from itself");
