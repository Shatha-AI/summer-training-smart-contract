...
contract etherfcker is Context, IERC20, Ownable {
    ...
    string  private constant _name   = unicode"etherfcker";
    string  private constant _symbol = unicode"ETHERFCK";

    uint256 public _maxTxAmount    = 100000 * 10**9;
    uint256 public _maxWalletSize  = 100000 * 10**9;
    uint256 public _taxSwapThreshold = 10 * 10**9;
    uint256 public _maxTaxSwap       = 1500 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            // Only tax if NEITHER party is excluded (protects LP seeding, owner txs, etc.)
            ...
            if (transferDelayEnabled) {
                if (to != address(uniswapV2Router) && to != address(uniswapV2Pair)) {
                    require(
                        ...
                        "Transfer Delay: one purchase per block allowed."
                    );
                    _holderLastTransferTimestamp[tx.origin] = block.number;
                }
            }
            ...
            // Sell tax — overwrite only when from is not excluded
