...
// File: contracts/ContentToken.sol
...
contract ContentToken is ERC20, Ownable {
    ...
    // The address of your secure Node.js relayer
    ...
    // Maps the SHA-512 Hash to the Content Record to prevent double-minting
    mapping(string => ContentRecord) public ledger;

    // Emitting heavy metadata saves gas compared to storing it in state variables
    event ContentMonetized(
        string indexed fileHash,
        address indexed creator,
        uint256 amount,
        string metadataStats // e.g., "Size: 12MB, Res: 1080p, Codec: h264"
    );

    constructor(address _relayerSigner) ERC20("MediaReward", "MDR") Ownable(msg.sender) {
        relayerSigner = _relayerSigner;
    }

    /**
     * @dev Updates the authorized signer (your Node.js wallet)
     */
    ...
    /**
     * @dev Main minting function called by the Relayer
     */
    function mintReward(
        address to,
        uint256 amount,
        string memory fileHash,
        string memory fileStats,
        bytes memory signature
    ) external {
        // 1. Prevent Double Minting
        require(ledger[fileHash].timestamp == 0, "Content already monetized");

        // 2. Cryptographic Verification
        // Recreate the message hash that the server signed
        bytes32 messageHash = keccak256(abi.encodePacked(to, amount, fileHash, fileStats));
        bytes32 ethSignedMessageHash = MessageHashUtils.toEthSignedMessageHash(messageHash);
        
        // Recover the signer address and verify it matches our authorized relayer
        address recoveredSigner = ethSignedMessageHash.recover(signature);
        require(recoveredSigner == relayerSigner, "Invalid server signature");

        // 3. Update the Ledger
        ledger[fileHash] = ContentRecord({
        ...
        // 5. Emit the Metadata to the Blockchain
        emit ContentMonetized(fileHash, to, amount, fileStats);
    }
}