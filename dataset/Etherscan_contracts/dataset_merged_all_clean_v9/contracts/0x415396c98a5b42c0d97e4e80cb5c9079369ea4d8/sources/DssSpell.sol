...
pragma solidity =0.8.16 >=0.5.12 >=0.8.0 ^0.8.16;
...
// src/dependencies/star-guard/StarGuardInit.sol
...
library StarGuardInit {
    function init(
        ...
        require(StarGuardLike_0(cfg.starGuard).wards(pauseProxy) == 1, "StarGuardInit/pauseProxy-not-authorized");
        require(StarGuardLike_0(cfg.starGuard).subProxy() == address(cfg.subProxy), "StarGuardInit/subProxy-does-not-match");
        require(cfg.maxDelay > 0, "StarGuardInit/invalid-maxDelay");
        (address addr,,) = StarGuardLike_0(cfg.starGuard).spellData();
        require(addr == address(0), "StarGuardInit/unexpected-plotted-spell");

        StarGuardLike_0(cfg.starGuard).file("maxDelay", cfg.maxDelay);
...
contract DssSpellAction is DssAction {
    ...
    function actions() public override {
        ...
        StarGuardLike_1(SPARK_STARGUARD).plot(SPARK_SPELL, SPARK_SPELL_HASH);
        ...
        StarGuardLike_1(GROVE_STARGUARD).plot(GROVE_SPELL, GROVE_SPELL_HASH);
    }
