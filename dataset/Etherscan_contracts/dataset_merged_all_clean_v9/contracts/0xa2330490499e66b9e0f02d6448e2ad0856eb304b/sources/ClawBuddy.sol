...
contract ClawBuddy is ERC721, ERC2981, Ownable, ReentrancyGuard, Pausable {
    using ECDSA for bytes32;
    using MessageHashUtils for bytes32;
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 10_000;
    uint256 public currentSupplyCap = 1_000;
    ...
    uint256 private _highestMintedTokenId;
    ...
    function mint(
        ...
        require(tokenId >= 1 && tokenId <= currentSupplyCap, "Token ID out of range");
        require(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(tokenId, msg.sender, address(this), block.chainid)
        );
        ...
        _totalMinted += 1;
        if (tokenId > _highestMintedTokenId) {
            _highestMintedTokenId = tokenId;
        }

        // Interaction (external call via onERC721Received)
    ...
    function setSigner(address _signer) external onlyOwner {
        ...
        emit SignerUpdated(signerAddress, _signer);
    ...
    function setMaxPerWallet(uint256 _max) external onlyOwner {
        ...
        emit MaxPerWalletUpdated(maxPerWallet, _max);
        maxPerWallet = _max;
    }

    function setSupplyCap(uint256 _cap) external onlyOwner {
        require(_cap <= MAX_SUPPLY, "Exceeds max supply");
        require(_cap >= _highestMintedTokenId, "Below highest minted token ID");
        currentSupplyCap = _cap;
    }
