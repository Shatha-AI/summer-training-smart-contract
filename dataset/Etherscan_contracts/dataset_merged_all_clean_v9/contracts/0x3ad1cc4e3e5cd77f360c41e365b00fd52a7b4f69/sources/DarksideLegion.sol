...
// src/DarksideLegion.sol

/**
 * @title DarksideLegion
 * @author EPEP Team
 * @notice ERC-721 NFT contract for the $EPEP Darkside Legion collection
 * 
 * Collection: 203 total NFTs
 * - 199 public mint (10 hidden 1/1 commanders + 189 generative, lucky dip with delayed reveal)
 * - 4 reserved (3 team + 1 artist, minted directly to wallets)
 * 
 * Features:
 * - ETH-only mint (~$499 equivalent, set as fixed ETH price at launch)
 * - No per-wallet mint limit
 * - Public mint only (no allowlist / no discount)
 * - Delayed reveal mechanism
 * - ERC-2981 royalties (5%)
 * - Revenue share integration for 1/1 holders
 */
contract DarksideLegion is 
    ERC721, 
    ERC721Enumerable, 
    ERC721Royalty, 
    Ownable, 
    ReentrancyGuard 
{
    ...
    uint256 public constant MAX_SUPPLY = 203;
    uint256 public constant PUBLIC_SUPPLY = 199;
    uint256 public constant TEAM_RESERVE = 4;
    ...
    // Revenue share tracking — the 14 special 1/1 token IDs
    // Set after reveal when we know which token IDs are 1/1s
    uint256[] public specialTokenIds;
    bool public specialTokensLocked = false;

    // Withdraw address (team multi-sig)
    ...
    event SpecialTokensSet(uint256[] tokenIds);
    event PriceUpdated(uint256 newMintPrice);
    ...
    /**
     * @param _mintPrice Mint price in wei (e.g., 0.19 ether for ~$499)
     * @param _preRevealTokenURI URI for pre-reveal placeholder metadata
     * @param _withdrawAddr Team multi-sig wallet for ETH withdrawals
    ...
    constructor(
        ...
        ERC721("EPEP: Darkside Legion", "EPEP") 
        Ownable(msg.sender) 
    {
        mintPrice = _mintPrice;
        _preRevealURI = _preRevealTokenURI;
    ...
    /**
     * @notice Public mint — open to everyone when mint is active
     * @param quantity Number of NFTs to mint
     */
    function mint(uint256 quantity) external payable nonReentrant {
        require(mintActive, "Mint not active");
        require(quantity > 0, "Quantity must be > 0");
        require(publicMinted + quantity <= PUBLIC_SUPPLY, "Exceeds public supply");
    ...
    /**
     * @notice Team reserve mint — owner only, mints directly to specified addresses
     * @param to Array of recipient addresses (max 4)
     */
    function teamMint(address[] calldata to) external onlyOwner {
        require(teamMinted + to.length <= TEAM_RESERVE, "Exceeds team reserve");

        for (uint256 i = 0; i < to.length; i++) {
            teamMinted++;
            // Team tokens use IDs 200-203 (after public supply)
            uint256 tokenId = PUBLIC_SUPPLY + teamMinted;
    ...
    /**
     * @notice Returns token URI — pre-reveal placeholder or revealed metadata
     */
    ...
    /**
     * @notice Reveal the collection — sets the real base URI
     * @param baseURI IPFS base URI (e.g., "ipfs://Qm.../")
     */
    ...
    /**
     * @notice Update base URI post-reveal if needed (e.g., migration)
    ...
    /**
     * @notice Update pre-reveal URI
     * @param preRevealURI New placeholder URI
     */
    ...
    /**
     * @notice Set the token IDs that are special 1/1 characters (for revenue share)
     * @dev Called after reveal. Includes the 10 hidden commanders + 4 team tokens = 14 total
     * @param tokenIds Array of 14 token IDs
     */
    function setSpecialTokenIds(uint256[] calldata tokenIds) external onlyOwner {
        require(!specialTokensLocked, "Special tokens already locked");
        require(tokenIds.length == 14, "Must be exactly 14 token IDs");

        delete specialTokenIds;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // Verify token exists
            _requireOwned(tokenIds[i]);
            specialTokenIds.push(tokenIds[i]);
        }

        emit SpecialTokensSet(tokenIds);
    }

    /**
     * @notice Permanently lock the special token IDs — no further changes
     */
    function lockSpecialTokenIds() external onlyOwner {
        require(specialTokenIds.length == 14, "Special tokens not set");
        specialTokensLocked = true;
    }

    /**
     * @notice Check if a token ID is a special 1/1
     */
    function isSpecialToken(uint256 tokenId) external view returns (bool) {
        for (uint256 i = 0; i < specialTokenIds.length; i++) {
            if (specialTokenIds[i] == tokenId) return true;
        }
        return false;
    }

    /**
     * @notice Get current owners of all 14 special tokens (for revenue share distribution)
     * @return owners Array of 14 addresses
     */
    function getSpecialTokenOwners() external view returns (address[] memory) {
        address[] memory owners = new address[](specialTokenIds.length);
        for (uint256 i = 0; i < specialTokenIds.length; i++) {
            owners[i] = ownerOf(specialTokenIds[i]);
        }
        return owners;
    }
    ...
    /**
     * @notice Withdraw all ETH to the team multi-sig
     */
