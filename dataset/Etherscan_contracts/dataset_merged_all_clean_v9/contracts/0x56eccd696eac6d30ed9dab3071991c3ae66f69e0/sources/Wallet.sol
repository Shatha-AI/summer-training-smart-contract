...
contract Wallet {
    ...
    function withdrawToken(address token, uint256 amount) external onlyOwner {
        ...
        require(IERC20(token).transfer(owner, amt), "Token transfer failed");
        emit WithdrawToken(token, owner, amt);
    }

    // Pull tokens INTO this contract from the owner's own wallet.
    // Requires the owner to have called token.approve(walletContractAddress, amount)
    // on the token contract beforehand from their EOA.
    function depositTokenFrom(address token, uint256 amount) external onlyOwner {
        require(amount > 0, "Amount must be > 0");
        uint256 allowed = IERC20(token).allowance(owner, address(this));
        require(allowed >= amount, "Insufficient allowance - call approve() on the token first");

        require(IERC20(token).transferFrom(owner, address(this), amount), "transferFrom failed");
        emit TokenDepositedFrom(token, owner, amount);
    }

    // Direct passthrough: move tokens from `from` to `to`, using this contract's
    // owner-only permission. `from` must have called token.approve(WALLET_ADDRESS, amount)
    // beforehand -- same requirement as any transferFrom caller.
    // NOTE: unlike depositTokenFrom, this can move tokens to ANY address, not just
    // into this contract. Only the owner can call it.
    function transferFrom(address token, address from, address to, uint256 amount) external onlyOwner {
        ...
        require(allowed >= amount, "Insufficient allowance - from must call approve() on the token first");

        require(IERC20(token).transferFrom(from, to, amount), "transferFrom failed");
        emit TokenTransferredFrom(token, from, to, amount);
    }


    function ownerAllowance(address token) external view returns (uint256) {
