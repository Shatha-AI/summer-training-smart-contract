...
// File: contracts/contracts/src/RewardHub.sol
...
/**
 * @title RewardHub：算力产出 + 共振/NFT 池（v3 合并 · UUPS 可升级）
 * @author suni
 * @notice 合并 MiningController + PoolManager：
 *         - 静态:算力占比×排放,用户自领;动态:纯团队奖沿链(代数≤8 + 级差)分发,余量转基金会;3× 出局
 *         - 共振池:V5/V6 惰性等额释放;NFT 池:入金3%+出金2%累积达阈值→均分节点持有者
 */
contract RewardHub is
    IRewardHub,
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
...
{
    uint256 private constant ACC_MINING = 1e12;
    uint256 private constant ACC_RESONANCE = 1e18;
    address private constant BLACKHOLE = 0x000000000000000000000000000000000000dEaD;

    // ---- 共用引用 ----
    IERC20 public suni;
    IRegistry public registry; // 邀请关系 + 团队等级
    INodeNFT public nodeNft;
    address public foundation; // 动态余量与超额去向

    mapping(address => bool) public authorized; // 可调 addHash（Vault / OperatorEntry）
    mapping(address => bool) public funders; // 可调 fundNftPool / fundResonance（Vault）

    // ---- 算力产出（Mining）----
    uint256 public staticRatePerSec; // sUNI/sec
    uint256 public dynamicRatePerSec; // sUNI/sec
    uint256 public totalHash; // 全网有效算力
    uint256 public accStatic; // 静态每单位算力累计(×ACC_MINING)
    uint256 public accDynBase; // 动态基数每单位算力累计(×ACC_MINING)
    ...
    function initialize(
        ...
        nftDistributeThreshold = nftThreshold_;
    ...
    function setRefs(address registry_, address nodeNft_, address foundation_) external onlyOwner {
        registry = IRegistry(registry_);
    ...
    function _updatePool() internal {
        ...
        if (dt == 0) return;
        if (totalHash > 0) {
            accStatic += (staticRatePerSec * dt * ACC_MINING) / totalHash;
            accDynBase += (dynamicRatePerSec * dt * ACC_MINING) / totalHash;
        }
        lastUpdate = block.timestamp;
    }
    ...
    function _settleStatic(address u) internal {
        ...
        if (a.hashWeight > 0) {
            a.staticClaimable += (a.hashWeight * (accStatic - a.debtStatic)) / ACC_MINING;
        }
    ...
    function _settleDynBase(address u) internal {
        ...
        uint256 bbase;
        if (a.hashWeight > 0) {
            bbase = (a.hashWeight * (accDynBase - a.debtDynBase)) / ACC_MINING;
        }
    ...
    function _distributeDynamic(address producer, uint256 bbase) internal {
        uint256 depth = registry.maxUplineDepth();
        address up = registry.referrer(producer);
        ...
        for (uint256 g = 1; g <= depth; g++) {
            ...
            if (users[up].active) {
                if (g <= 8) {
                    uint256 maxg = _maxGen(registry.directCount(up));
                    if (g <= maxg) {
                        uint256 rate = (g == 1) ? 10 : 5;
                        amt += (bbase * rate) / 100;
                    }
                }
                uint256 lvRate = registry.rewardRateOf(registry.levelOf(up));
            ...
            up = registry.referrer(up);
        }

        uint256 remainder = bbase - allocated;
        if (remainder > 0 && foundation != address(0)) {
            suni.transfer(foundation, remainder);
        }
    }

    /// @dev 支付动态收益,受收款方出局额度封顶;超额转基金会;达额度即出局
    ...
    function claim() external nonReentrant {
        ...
        a.staticClaimable = 0;
        _pay(msg.sender, amt, msg.sender);
    ...
    function pendingStatic(address u) external view returns (uint256) {
        ...
        uint256 dt = block.timestamp - lastUpdate;
        if (dt > 0 && totalHash > 0) {
            acc += (staticRatePerSec * dt * ACC_MINING) / totalHash;
        }
        return a.staticClaimable + (a.hashWeight * (acc - a.debtStatic)) / ACC_MINING;
    }
    ...
    function _updateResonance() internal {
        ...
        if (dt > 0) {
            if (resonanceMemberCount > 0) {
                accResonancePerMember += (resonanceRatePerSec * dt * ACC_RESONANCE) / resonanceMemberCount;
            }
            resonanceLastUpdate = block.timestamp;
        }
    }

    /// @notice Registry 在用户跨入 V5 时回调(去重、单调)
    ...
    function claimResonance() external nonReentrant {
        require(isResonanceMember[msg.sender], "not member");
        _updateResonance();
        uint256 amount = (accResonancePerMember - resonanceDebt[msg.sender]) / ACC_RESONANCE;
    ...
    /// @notice 注入共振池(sUNI 已由调用方转入本合约);仅记账,释放靠惰性指数
