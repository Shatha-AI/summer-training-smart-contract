 ...
 *        - Sell SCENT: 10% of the JPYC proceeds → sent to treasury wallet
 *
 *      Security Features:
...
contract ScentTokenExchange is ReentrancyGuard {
     ...
     * @param _side Buy or Sell (from the maker's perspective relative to SCENT)
     */
        ...
        returns (bytes32)
    {
        ...
        // [FIX #4] Use abi.encode instead of abi.encodePacked to prevent hash collisions
        ...
        emit OrderCreated(orderId, msg.sender, _makerToken, _takerToken, _makerAmount, _takerAmount, OrderType.LIMIT, _side);
     ...
     * @dev [FIX #2] Added _minimumReceived for slippage/front-running protection.
        ...
        nonReentrant
    {
        ...
        // [FIX #2] Slippage protection: revert if received less than minimum
     ...
     *      [FIX #1] Fee is only charged when SCENT is being sold (side-aware).
     *      [FIX #2] Added _minimumReceived for front-running protection.
     ...
     * @dev [FIX #3] Returns escrowed maker tokens back to the maker.
        ...
        nonReentrant
    {
        ...
        // [FIX #3] Return remaining escrowed tokens to maker
        uint256 remainingEscrow = order.makerAmount - order.filledMakerAmount;
        if (remainingEscrow > 0) {
            IERC20(order.makerToken).safeTransfer(order.maker, remainingEscrow);
        }
    ...
    /**
     * @notice Calculate the fee for a given JPYC amount on a sell order.
     * @param _jpycAmount The JPYC amount to calculate fee for
     * @return feeAmount The fee in JPYC (10%)
     */
    function calculateSellFee(uint256 _jpycAmount) public pure returns (uint256 feeAmount) {
        feeAmount = (_jpycAmount * SELL_FEE_BPS) / BPS_DENOMINATOR;
    }
    ...
    /**
     * @dev [FIX #1] Determine if this trade involves selling SCENT.
     *      Fee is ONLY charged when SCENT is being sold, NOT when buying.
     *
     *      The key insight: we use the order's `side` field to determine direction.
     *        - side == Sell → maker is selling SCENT → fee on maker's JPYC proceeds
     *        - side == Buy  → maker is buying SCENT → taker is selling SCENT → fee on taker's JPYC proceeds
     *
     *      Previously, this function returned true whenever SCENT appeared in either
     *      makerToken or takerToken, which incorrectly charged fees on Buy SCENT orders too.
     *      Now it correctly identifies only the sell side.
     */
    function _isScentSell(Order storage _order) internal view returns (bool) {
        // Sell order: maker is selling SCENT (makerToken == SCENT)
        if (_order.side == Side.Sell && _order.makerToken == scentToken) return true;
        // Buy order: taker is selling SCENT to fill the buy (takerToken == SCENT)
        if (_order.side == Side.Buy && _order.takerToken == scentToken) return true;
        return false;
    }

    /**
     * @dev Execute the atomic token swap between maker and taker.
     *      [FIX #1] Fee is only charged on SCENT sell side (not buy).
     *      [FIX #3] Maker tokens are paid from escrow (this contract), not from maker's wallet.
     *
    ...
    function _executeSwap(
        ...
        // ─── [AUDIT FIX #1] CEI Pattern: Update state BEFORE external calls ───
        // This follows the Checks-Effects-Interactions pattern to eliminate
        // reentrancy risk even without nonReentrant (defense in depth).
        ...
        if (_isScentSell(_order)) {
            if (_order.side == Side.Sell) {
                // Maker sells SCENT (escrowed), receives JPYC from taker
                // Fee is deducted from the JPYC that the taker pays (maker's JPYC proceeds)
                feeAmount = calculateSellFee(_fillTakerAmount);
                uint256 makerReceives = _fillTakerAmount - feeAmount;

                // Transfer SCENT from escrow (this contract) to taker
                IERC20(_order.makerToken).safeTransfer(_taker, _fillMakerAmount);

                // Transfer JPYC from taker: (amount - fee) to maker, fee to treasury
                ...
                // Taker receives full SCENT amount (fee is on JPYC side)
                takerActualReceived = _fillMakerAmount;
            } else {
                // Buy order filled: taker sells SCENT, receives JPYC from escrow
                // Fee is deducted from the JPYC (makerToken) that the taker receives
                feeAmount = calculateSellFee(_fillMakerAmount);
                uint256 takerReceives = _fillMakerAmount - feeAmount;

                // Transfer JPYC from escrow (this contract): (amount - fee) to taker, fee to treasury
            ...
            // No fee — Buy SCENT = zero fee
            // Transfer makerToken from escrow to taker
            IERC20(_order.makerToken).safeTransfer(_taker, _fillMakerAmount);
        ...
        // ─── [AUDIT FIX #2] Gas optimization: accumulate fee in memory, write once ───
        // totalFeesCollected is updated here (single SSTORE per call).
        // In executeMarketOrder loops, this is called per iteration, but each
        // _executeSwap call is a separate fill so one SSTORE per fill is optimal.
        if (feeAmount > 0) {
