...
/**
 * @title UNI→U 计价预言机（v3 · UUPS 可升级）
 * @author suni
 * @notice 基于 UNI/USDT Uniswap V2 交易对的累积价格做 TWAP,将 UNI 折算成 U(18 位)
 * @dev 需外部定期调用 update() 累积价格窗口;首版简易 TWAP,生产前建议加长窗口与偏离校验
 */
contract PriceOracle is IPriceOracle, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    IUniswapV2Pair public pair; // UNI/USDT 交易对
    bool public uniIsToken0;
    uint256 public uniDecimalsFactor;

    uint256 public priceCumulativeLast;
    uint32 public blockTimestampLast;
    uint256 public priceAverage; // UQ112x112:每 1 单位 UNI 值多少 U(按最小单位)
    uint256 public minWindow;
    ...
    function initialize(address owner_, address pair_, address uni, uint8 uniDecimals) public initializer {
