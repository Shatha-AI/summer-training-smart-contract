...
// File: contracts/contracts/src/NodeNFT.sol




/**
 * @title 节点 NFT（v3 · UUPS 可升级）
 * @author suni
 * @notice 私募节点凭证,总量上限可配;Enumerable 便于 NFT 池按持有者均分(有界循环)
 */
contract NodeNFT is
    ERC721Upgradeable,
    ...
    INodeNFT
{
    ...
    function setMinter(address account, bool v) external onlyOwner {
        minters[account] = v;
    }

    function setMaxNodes(uint256 n) external onlyOwner {
        require(n >= _nextId && n <= 50, "bad max");
        maxNodes = n;
    }

    /// @notice 铸造下一枚节点,受铸造白名单控制,超过上限回退
    function mint(address to) external override returns (uint256 tokenId) {
        require(minters[msg.sender] || msg.sender == owner(), "not minter");
        require(_nextId < maxNodes, "sold out");
        tokenId = _nextId++;
        _safeMint(to, tokenId);
    }
    ...
    function totalSupply()
        public
        view
        override(ERC721EnumerableUpgradeable, INodeNFT)
        returns (uint256)
    {
    ...
    function tokenByIndex(uint256 index)
        ...
        returns (uint256)
    {
        return ERC721EnumerableUpgradeable.tokenByIndex(index);
    }

    function ownerOf(uint256 tokenId)
        public
        view
        override(ERC721Upgradeable, IERC721, INodeNFT)
        returns (address)
    {
