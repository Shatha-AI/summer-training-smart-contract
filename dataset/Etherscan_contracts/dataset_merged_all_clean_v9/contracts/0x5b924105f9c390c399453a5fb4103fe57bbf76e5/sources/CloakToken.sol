...
/*
CLOAK ($CLOAK) — the privacy layer for the agent economy.

Utility token for the Cloak private-x402 stack. The swap fee is taken by the
CloakHook (not inside this token) and routed to the treasury that funds the
privacy infrastructure (relayers, facilitator, proving).

 X: https://x.com/Privacy_Cloak
*/
pragma solidity 0.8.24;

contract CloakToken {
    string public constant name     = "Cloak";
    ...
    uint256 public maxBuy    = TOTAL_SUPPLY / 200;       // 0.5%
    uint256 public maxWallet = TOTAL_SUPPLY / 200;       // 0.5%
    ...
    function setLimits(uint256 b, uint256 w) external onlyOwner { require(b >= TOTAL_SUPPLY / 200 && w >= TOTAL_SUPPLY / 200, "limit too low"); maxBuy = b; maxWallet = w; }
